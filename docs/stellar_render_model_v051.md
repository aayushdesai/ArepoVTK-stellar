...
v051a uses smooth density, temperature, and geometry gates. Full velocity-vector
classification is deferred to the next scene-format revision; the current GPU
scene stores only position, density, and temperature.

## Shot classes

- `merger`: compact thermal material for the binary and coalescence.
- `disk`: a thin cylindrical layer around the tracked angular-momentum axis.
- `outflow`: a broad bipolar cone with an independent large spatial scale.
...
of view near 1e10 cm. Outflow pilots scale from 5e10 cm to about 5e11 cm using
...
gold and warm white to pale blue. The disk uses warm gold and the outflow uses
cobalt. Teal and pink are excluded. Front-to-back opacity prevents unrelated
line-of-sight material from adding without limit.

## Validation

Phase 1 validation job 63693881 ran on eta294 with `Arepo_Env`. It proved:
