#!/usr/bin/env python3
"""Build no-clobber v053f 43-pose rotational-fraction acceptance manifests."""
...
OUTPUT_SCHEMA = "# schema=stellar_gpu_view_manifest_v053f"
EXPECTED_POSES = 43
...
def build_manifest_row(
    ...
    if len(output) != 52:
        raise AssertionError(f"v053f row has {len(output)} columns, expected 52")
    return output
...
def main() -> int:
    ...
    for wave in args.wave:
        for snapshot, rows in sorted(rows_by_snapshot.items()):
            ...
            manifest = destination / "view_manifest_v053f.tsv"
    ...
    package = {
        "schema": "stellar_gpu_v053f_acceptance_package_v001",
    ...
    print(
        "STELLAR_GPU_V053F_ACCEPTANCE_PACKAGE_OK "
