#!/usr/bin/env python3
"""Build no-clobber v053i hero manifests for the exact reviewed poses."""
...
OUTPUT_SCHEMA = "# schema=stellar_gpu_view_manifest_v053i"
EXPECTED_POSES = 43
...
def build_row(
    ...
    calibration_sha256: str, material_tau: float, outflow_emission: float,
) -> list[str]:
    output = list(fields)
    output[0] = str(index)
    output[3] = f"hero_v079__{fields[3]}"
...
def main() -> int:
    ...
    parser.add_argument("--material-optical-depth", type=float, required=True)
    parser.add_argument("--outflow-emission", type=float, required=True)
    ...
    if not args.material_optical_depth > 0 or not args.outflow_emission > 0:
        raise SystemExit("hero optical depth and outflow emission must be positive")

    bundle = json.loads(args.review_bundle.read_text(encoding="utf-8"))
    if calibration.get("schema") != "stellar_hero_calibration_v079":
        raise SystemExit("unexpected hero calibration schema")
    if (not args.calibration_probe and
            calibration.get("review_bundle_sha256") !=
            args.review_bundle_sha256):
        raise SystemExit("hero calibration is bound to a different review bundle")
    ...
    for snapshot, rows in sorted(rows_by_snapshot.items()):
        ...
        manifest = destination / "view_manifest_v053i.tsv"
        lines = [OUTPUT_SCHEMA]
        for local_index, fields in enumerate(rows):
            lines.append(
                " ".join(
                    build_row(
                        fields, local_index, style, calibration,
                        calibration_sha256, args.material_optical_depth,
                        args.outflow_emission,
                    )
                )
            )
        manifest.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    package = {
        "schema": "stellar_gpu_v053i_hero_package_v001",
        ...
        "render_program": "hero_material_outflow_v079",
        "components": ["material_column", "outward_mass_flux"],
        "static_structure_weights": False,
        "material_optical_depth": args.material_optical_depth,
        "outflow_emission": args.outflow_emission,
    ...
    package_path = args.output_root / "hero_package.json"
    package_path.write_text(
        json.dumps(package, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(
        "STELLAR_GPU_V053I_HERO_PACKAGE_OK "
