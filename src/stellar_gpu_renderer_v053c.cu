...
__host__ __device__ void copyCellValues(const ArepoStellarCell &cell,
...
__host__ __device__ RayResult renderRay(const DeviceScene &scene, uint64_t rayIndex)
{
  ...
  for(int iteration = 0; iteration < kMaxCellsPerRay && currentT < ray.t_max; iteration++) {
    ...
    for(uint64_t edgeIndex = first; edgeIndex < last; edgeIndex++) {
      ...
      const ArepoStellarCell &neighborCell = scene.cells[candidateCell];
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053c") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053c schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    if(!stellarGpuProfileContractValidV053c(
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053c schema.");
  if(views.empty())
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053c.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053c supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      report << "renderer_version=stellar_v053c\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053c\n";
    ...
    summary << "renderer_version=stellar_v053c\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053c\n";
