...
#pragma omp parallel for schedule(static)
  for(long long pixelIndex = 0; pixelIndex < static_cast<long long>(results.size()); pixelIndex++) {
    ...
    stellarDecodePhysicalMomentsV078(
        results[i].rgb, physicalOptical, linearRgb);
    float mapped[3];
    for(int channel = 0; channel < 3; channel++)
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053h") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053h schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    if(!stellarGpuProfileContractValidV053h(
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053h schema.");
  if(views.empty())
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053h.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053h supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      writeTga(outputPrefix + ".tga", gpuResults,
               ...
               view.exposure, view.black_point, view.saturation,
               view.display_brightness, view.physical_optical);
      ...
      report << "renderer_version=stellar_v053h\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053h\n";
    ...
    summary << "renderer_version=stellar_v053h\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053h\n";
