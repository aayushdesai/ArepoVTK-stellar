...
void setMotionBudgetOption(const std::string &name, const std::string &text,
                           StellarDirectorMotionBudgetV063 *budget)
{
  const double value = parseFinite(text, name);
...
int main(int argc, char **argv)
{
  try {
    ...
    const std::set<std::string> path_options = {
        "--landmarks", "--shots", "--visual-calibration", "--path", "--diagnostics",
        "--manifest", "--preview"};
    ...
    StellarDirectorMotionBudgetV063 motion_budget = {0.25, 1.0, 5.0};
    ...
    if(!stellarDirectorValidMotionBudgetV063(motion_budget))
    ...
    std::string error;
    if(!stellarDirectorLoadLandmarksV063(landmarks_input, &landmarks, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadShotsV063(shots_input, &shots, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadVisualCalibrationV063(
           calibration_input, &calibration, &error))
      throw std::runtime_error(error);
    StellarDirectorResultV063 result;
    if(!stellarDirectorBuildV063(landmarks, shots, calibration, filter,
                                 motion_budget,
                                 &result, &error))
      throw std::runtime_error(error);
    const StellarDirectorMotionAssessmentV063 motion_assessment =
        stellarDirectorAssessMotionV063(result, motion_budget);
    ...
    std::string manifest = stellarDirectorManifestV063(
        result, shots, calibration, filter, motion_budget, motion_assessment,
        landmarks_path, shots_path, calibration_path, dry_run);
    ...
    if(path_given && motion_assessment.passed)
      writeNoClobber(path_path, stellarDirectorPathTableV063(
          result, landmarks_path, shots_path, calibration_path),
          "camera path");
    writeNoClobber(diagnostics_path,
                   stellarDirectorDiagnosticsTableV063(result),
                   "diagnostics");
    writeNoClobber(preview_path, stellarDirectorPreviewSvgV063(result),
                   "preview");
    writeNoClobber(manifest_path, manifest, "manifest");
    if(path_given && !motion_assessment.passed) {
      std::cerr << "STELLAR_CAMERA_DIRECTOR_V063_MOTION_REJECTED roll_deg_per_frame="
    ...
    std::cout << "STELLAR_CAMERA_DIRECTOR_V063_OK rows="
              << result.path.size() << " shots=" << shots.size()
              << " visual_calibration_rows=" << calibration.size()
              << " dry_run=" << (dry_run ? "true" : "false") << "\n";
    return 0;
  } catch(const std::exception &error) {
    std::cerr << "STELLAR_CAMERA_DIRECTOR_V063_ERROR " << error.what()
              << "\n";
    return 1;
  }
}
