...
struct SceneData {
  ...
  std::vector<ArepoStellarEdge> edges;
  std::vector<ArepoStellarRay> rays;
};

struct DeviceScene {
  const ArepoStellarCell *cells;
  const uint64_t *offsets;
  const ArepoStellarEdge *edges;
  const ArepoStellarRay *rays;
  const float *palette;
...
SceneData loadScene(const std::string &filename)
{
  ...
  if(std::memcmp(scene.header.magic, AREPO_STELLAR_SCENE_MAGIC,
                 std::strlen(AREPO_STELLAR_SCENE_MAGIC)) != 0 ||
     scene.header.version != AREPO_STELLAR_SCENE_VERSION ||
     scene.header.endian_marker != AREPO_STELLAR_SCENE_ENDIAN_MARKER ||
     scene.header.header_bytes != sizeof(ArepoStellarSceneHeader) ||
     scene.header.cell_bytes != sizeof(ArepoStellarCell) ||
     scene.header.edge_bytes != sizeof(ArepoStellarEdge) ||
     scene.header.ray_bytes != sizeof(ArepoStellarRay))
    throw std::runtime_error("Unsupported or corrupt GPU scene header.");
  const uint32_t requiredFlags = AREPO_STELLAR_REQUIRED_FIELD_FLAGS |
      AREPO_STELLAR_ZERO_LEGACY_ABSORPTION;
  if((scene.header.flags & requiredFlags) != requiredFlags)
    throw std::runtime_error("Scene does not match the v052 stellar physics contract.");
  if(scene.header.position_unit_cm != 1.0 ||
    ...
    throw std::runtime_error("v052 renderer requires canonical cgs scene values.");
...
__host__ __device__ inline int edgeCell(const ArepoStellarEdge &edge)
{
  const uint32_t encoded = edge.packed_neighbor & 0x7fffffffu;
  return encoded == 0 ? -1 : int(encoded - 1u);
}

__host__ __device__ inline bool edgeContributes(const ArepoStellarEdge &edge)
{
                                               ...
                                               int *handled, uint32_t *status)
{
  ...
  for(uint64_t outer = first; outer < last; outer++) {
    const ArepoStellarEdge &outerEdge = scene.edges[outer];
    ...
    for(uint64_t inner = innerFirst; inner < innerLast; inner++) {
      const ArepoStellarEdge &innerEdge = scene.edges[inner];
...
__host__ __device__ bool interpolateIdw(const DeviceScene &scene, int parent,
                                        ...
                                        uint32_t *status)
{
  const ArepoStellarCell &parentCell = scene.cells[parent];
  if(fabs(point[0] - parentCell.position[0]) <= 1.0e-11 &&
    ...
    *density = parentCell.density_log10_plus_10;
    *temperature = parentCell.temperature_kelvin;
    for(int component = 0; component < 3; component++)
      velocity[component] = parentCell.velocity_cm_per_s[component];
    return true;
  }

  float densitySum = 0.0f;
  float temperatureSum = 0.0f;
  float velocitySum[3] = {0.0f, 0.0f, 0.0f};
  ...
  for(int i = 0; i < handledCount; i++) {
    ...
    weightSum += weight;
    densitySum += weight * cell.density_log10_plus_10;
    temperatureSum += weight * cell.temperature_kelvin;
    for(int component = 0; component < 3; component++)
      velocitySum[component] += weight * cell.velocity_cm_per_s[component];
  }
  ...
  densitySum += parentWeight * parentCell.density_log10_plus_10;
  temperatureSum += parentWeight * parentCell.temperature_kelvin;
  for(int component = 0; component < 3; component++)
    velocitySum[component] += parentWeight * parentCell.velocity_cm_per_s[component];
  ...
  *temperature = temperatureSum / weightSum;
  for(int component = 0; component < 3; component++)
    velocity[component] = velocitySum[component] / weightSum;
...
__host__ __device__ RayResult renderRay(const DeviceScene &scene, uint64_t rayIndex)
{
  ...
  const ArepoStellarRay &ray = scene.rays[rayIndex];
  if(!ray.active) {
    ...
    if(length > 1.0e-11) {
      ...
      for(int sampleIndex = 0; sampleIndex < scene.samples_per_cell; sampleIndex++) {
        ...
        if(!interpolateIdw(scene, cell, point, handled, handledCount,
                           &density, &temperature, velocity, &result.status))
          break;
        if(density < 0.0f)
          density = 0.0f;
        const StellarOpticalSample optical =
            evaluateTransfer(scene, point, density, temperature, velocity);
...
ArepoStellarSceneHeader readSceneHeader(std::ifstream &input)
{
  ArepoStellarSceneHeader header;
  input.read(reinterpret_cast<char *>(&header), sizeof(header));
  if(!input.good())
    throw std::runtime_error("Cannot read scene header.");
  if(std::memcmp(header.magic, AREPO_STELLAR_SCENE_MAGIC,
                 std::strlen(AREPO_STELLAR_SCENE_MAGIC)) != 0 ||
     header.version != AREPO_STELLAR_SCENE_VERSION ||
     header.endian_marker != AREPO_STELLAR_SCENE_ENDIAN_MARKER ||
     header.header_bytes != sizeof(ArepoStellarSceneHeader) ||
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(!(axisNorm > 0.0) || !(view.material_radius_cm > 0.0f) ||
...
void validateRayHeader(const ArepoStellarSceneHeader &base,
                       const ArepoStellarSceneHeader &candidate,
                       bool expectRaysOnly)
{
  const bool raysOnly = (candidate.flags & AREPO_STELLAR_RAYS_ONLY) != 0;
  ...
  if(candidate.num_cells != base.num_cells ||
     ...
     (candidate.flags & AREPO_STELLAR_REQUIRED_FIELD_FLAGS) !=
         (base.flags & AREPO_STELLAR_REQUIRED_FIELD_FLAGS) ||
  ...
  try {
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v052a supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    const ArepoStellarSceneHeader baseHeader = readSceneHeader(firstInput);
    ...
    ArepoStellarCell *deviceCells = 0;
    uint64_t *deviceOffsets = 0;
    ArepoStellarEdge *deviceEdges = 0;
    ArepoStellarRay *deviceRays = 0;
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      ArepoStellarSceneHeader viewHeader = baseHeader;
      ...
      report << "renderer_version=stellar_v052a\n";
    ...
    summary << "renderer_version=stellar_v052a\n";
    ...
    summary << "compact_ray_bytes=" << sizeof(ArepoStellarRay) << "\n";
