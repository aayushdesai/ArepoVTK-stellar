...
void setMotionBudgetOption(const std::string &name, const std::string &text,
                           StellarDirectorMotionBudgetV060 *budget)
{
  const double value = parseFinite(text, name);
...
int main(int argc, char **argv)
{
  try {
    ...
    filter.minimum_half_extent_cm = 1.0e8;
    ...
    StellarDirectorMotionBudgetV060 motion_budget = {0.25, 1.0, 5.0};
    ...
    if(!stellarDirectorValidMotionBudgetV060(motion_budget))
    ...
    std::string error;
    if(!stellarDirectorLoadLandmarksV060(landmarks_input, &landmarks, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadShotsV060(shots_input, &shots, &error))
      throw std::runtime_error(error);
    StellarDirectorResultV060 result;
    if(!stellarDirectorBuildV060(landmarks, shots, filter, motion_budget,
                                 &result, &error))
      throw std::runtime_error(error);
    const StellarDirectorMotionAssessmentV060 motion_assessment =
        stellarDirectorAssessMotionV060(result, motion_budget);
    ...
    std::string manifest = stellarDirectorManifestV060(
        result, shots, filter, motion_budget, motion_assessment,
        landmarks_path, shots_path, dry_run);
    ...
    if(path_given && motion_assessment.passed)
      writeNoClobber(path_path, stellarDirectorPathTableV060(
          result, landmarks_path, shots_path), "camera path");
    writeNoClobber(diagnostics_path,
                   stellarDirectorDiagnosticsTableV060(result),
                   "diagnostics");
    writeNoClobber(preview_path, stellarDirectorPreviewSvgV060(result),
                   "preview");
    writeNoClobber(manifest_path, manifest, "manifest");
    if(path_given && !motion_assessment.passed) {
      std::cerr << "STELLAR_CAMERA_DIRECTOR_V060_MOTION_REJECTED roll_deg_per_frame="
    ...
    std::cout << "STELLAR_CAMERA_DIRECTOR_V060_OK rows="
              << result.path.size() << " shots=" << shots.size()
              << " dry_run=" << (dry_run ? "true" : "false") << "\n";
    return 0;
  } catch(const std::exception &error) {
    std::cerr << "STELLAR_CAMERA_DIRECTOR_V060_ERROR " << error.what()
              << "\n";
    return 1;
  }
}
