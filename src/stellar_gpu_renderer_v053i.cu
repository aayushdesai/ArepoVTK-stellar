...
struct DeviceScene {
  ...
  int render_program;
  StellarHeroCalibrationV079 hero_calibration;
  float exposure;
  float black_point;
  float saturation;
...
__host__ __device__ void accumulateHeroDiagnostics(
    ...
    RayResult *result)
{
  ...
  result->material_column_g_cm2 +=
      terms.material_density_g_cm3 * step;
  result->outflow_column_flux_g_cm_s +=
      terms.outward_mass_flux_g_cm2_s * step;
}
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053i") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053i schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    if(!(parser >> view.index >> view.scene_path >> view.palette_path >> view.output_prefix >>
         ...
         view.hero_calibration.material_optical_depth_at_reference >>
         view.hero_calibration.outflow_column_flux_reference_g_cm_s >>
         view.hero_calibration.outflow_emission_at_reference >>
    ...
    view.render_program = stellarRenderProgramFromNameV079(renderProgram);
    ...
    if(!stellarGpuProfileContractValidV053i(
    ...
    const bool physical = view.physical_transfer.channel !=
        STELLAR_PHYSICAL_CHANNEL_OPTICAL_V071;
    if(view.render_program == STELLAR_RENDER_PROGRAM_INVALID_V079)
    ...
    if(view.render_program ==
       ...
       (view.physical_transfer.channel !=
        ...
        !stellarHeroCalibrationValidV079(view.hero_calibration) ||
        !stellarHeroCalibrationSha256ValidV079(
            view.hero_calibration_sha256)))
      throw std::runtime_error("Invalid hero v079 render contract: " + line);
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053i schema.");
  if(views.empty())
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053i.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053i supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      report << "renderer_version=stellar_v053i\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053i\n";
      ...
      report << "hero_material_column_reference_g_cm2="
             << view.hero_calibration.material_column_reference_g_cm2 << "\n";
      report << "hero_material_optical_depth_at_reference="
             << view.hero_calibration.material_optical_depth_at_reference
             << "\n";
      report << "hero_outflow_column_flux_reference_g_cm_s="
             << view.hero_calibration.outflow_column_flux_reference_g_cm_s
             << "\n";
      report << "hero_outflow_emission_at_reference="
             << view.hero_calibration.outflow_emission_at_reference << "\n";
      report << "hero_calibration_sha256="
             << view.hero_calibration_sha256 << "\n";
      report << "hero_material_column_p50_g_cm2="
             << materialColumnP50 << "\n";
      report << "hero_material_column_p90_g_cm2="
             << materialColumnP90 << "\n";
      report << "hero_material_column_p95_g_cm2="
             << materialColumnP95 << "\n";
      report << "hero_outflow_column_flux_p50_g_cm_s="
             << outflowColumnFluxP50 << "\n";
      report << "hero_outflow_column_flux_p90_g_cm_s="
             << outflowColumnFluxP90 << "\n";
      report << "hero_outflow_column_flux_p95_g_cm_s="
    ...
    summary << "renderer_version=stellar_v053i\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053i\n";
    ...
    summary << "render_program_contract=v079_retained_v078_or_hero_material_outflow\n";
    summary << "hero_components=material_column,outward_mass_flux\n";
