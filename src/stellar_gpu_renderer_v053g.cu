...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053g") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053g schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    view.physical_optical.profile =
        stellarPhysicalOpticalProfileFromNameV077(physicalOpticalProfile);
    std::string extra;
    if(parser >> extra)
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053g schema.");
  if(views.empty())
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053g.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053g supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      report << "renderer_version=stellar_v053g\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053g\n";
    ...
    summary << "renderer_version=stellar_v053g\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053g\n";
    summary << "profile_contract=manifest_bound\n";
    summary << "physical_channel_contract=v072_all_24\n";
    summary << "physical_optical_contract=v077_legacy_v072_through_normalized_moment_v077\n";
