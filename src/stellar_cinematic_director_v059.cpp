...
bool poseForShot(const StellarCameraState &state,
                 const StellarDirectorShotV059 &shot,
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    StellarDirectorLandmarkRowV059 row = {};
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_cinematic_shots_v059", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    ...
    shot.has_transition_in = value[11] != "-";
    if((shot.has_orbit_degrees && !parseFinite(value[8], &orbit_degrees)) ||
       ...
       (shot.has_transition_in &&
        !parseUnsigned(value[11], &shot.transition_end_snapshot)))
    ...
    if(!shots->empty()) {
      const StellarDirectorShotV059 &previous = shots->back();
  ...
  if(!schema_seen)
    return fail(error, "Missing schema=stellar_cinematic_shots_v059.");
  if(shots->empty())
    ...
    StellarDirectorResultV059 *result, std::string *error)
{
  ...
  for(std::size_t index = 0; index < landmarks.size(); ++index) {
    ...
    StellarDirectorPathRowV059 row = {};
    ...
    if(shot_index > 0 && shots[shot_index].has_transition_in &&
      ...
      if(!poseForShot(state, shots[shot_index - 1], previous_start,
                      previous_end, &previous_pose) ||
  ...
  for(std::size_t index = 0; index < result->path.size(); ++index) {
    const StellarDirectorPathRowV059 &row = result->path[index];
    StellarDirectorDiagnosticRowV059 diagnostic = {};
    ...
    if(index > 0) {
      const StellarDirectorPathRowV059 &previous = result->path[index - 1];
...
StellarDirectorMotionAssessmentV059 stellarDirectorAssessMotionV059(
    ...
    const StellarDirectorMotionBudgetV059 &budget)
{
  StellarDirectorMotionAssessmentV059 assessment = {};
  assessment.passed = stellarDirectorValidMotionBudgetV059(budget);
  ...
  assessment.passed = assessment.passed &&
      assessment.maximum_roll_deg_per_frame <= budget.max_roll_deg_per_frame &&
    ...
    const std::string &landmarks_path, const std::string &shots_path)
{
  ...
  output << "# schema=stellar_camera_path_v055\n"
         << "# director_schema=stellar_cinematic_director_v059\n"
...
{
  std::ostringstream output;
  output << "# schema=stellar_camera_diagnostics_v059\n"
         << "# snapshot time shot transition_from transition_to "
            ...
            "log_zoom_acceleration material_occupancy "
    ...
    bool dry_run)
{
  std::ostringstream output;
  output << std::setprecision(17)
         << "schema=stellar_cinematic_direction_manifest_v059\n"
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    output << "shot." << index << ".name=" << shot.name << "\n"
           ...
           << "\n"
           << "shot." << index << ".easing="
           << stellarDirectorEasingNameV059(shot.easing) << "\n";
  }
  return output.str();
}
