...
#include "stellar_gpu_profile_contract_v053j.h"
...
struct DeviceScene {
  ...
  int render_program;
  StellarHeroProjectionCalibrationV080 hero_calibration;
  float exposure;
  float black_point;
  float saturation;
...
__host__ __device__ void accumulateHeroProjection(
    ...
    float temperature, const float velocity[3], float step,
    RayResult *result)
{
  ...
  result->material_column_g_cm2 +=
      sample.material_density_g_cm3 * step;
  result->styled_rotation_column_g_cm2 +=
      sample.styled_rotation_density_g_cm3 * step;
  result->hot_material_column_g_cm2 +=
      sample.hot_material_density_g_cm3 * step;
  result->outflow_column_flux_g_cm_s +=
      sample.outward_mass_flux_g_cm2_s * step;
}
...
__host__ __device__ RayResult renderRay(const DeviceScene &scene, uint64_t rayIndex)
{
  ...
  result.material_column_g_cm2 = 0.0f;
  result.styled_rotation_column_g_cm2 = 0.0f;
  result.hot_material_column_g_cm2 = 0.0f;
  result.outflow_column_flux_g_cm_s = 0.0f;
      ...
      for(int sampleIndex = 0; sampleIndex < scene.samples_per_cell; sampleIndex++) {
        ...
        if(scene.render_program ==
          ...
          accumulateHeroProjection(
              scene, point, density, temperature, velocity, step, &result);
        } else {
  ...
  if(scene.render_program ==
    ...
    moments.material_column_g_cm2 = result.material_column_g_cm2;
    moments.styled_rotation_column_g_cm2 =
        result.styled_rotation_column_g_cm2;
    moments.hot_material_column_g_cm2 = result.hot_material_column_g_cm2;
    moments.outflow_column_flux_g_cm_s =
        result.outflow_column_flux_g_cm_s;
    stellarDecodeHeroProjectionV080(
        moments, scene.hero_calibration, result.rgb);
  }
...
float positiveQuantile(std::vector<float> values, float quantile)
{
...
struct ViewSpec {
  ...
  int render_program;
  StellarHeroProjectionCalibrationV080 hero_calibration;
  std::string hero_calibration_sha256;
};
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053j") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053j schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    if(!(parser >> view.index >> view.scene_path >> view.palette_path >> view.output_prefix >>
         ...
         view.hero_calibration.material_column_reference_g_cm2 >>
         view.hero_calibration.material_display_gain >>
         view.hero_calibration.outflow_column_flux_reference_g_cm_s >>
         view.hero_calibration.outflow_display_gain >>
         view.hero_calibration_sha256))
    ...
    view.render_program = stellarRenderProgramFromNameV080(renderProgram);
    ...
    if(!stellarGpuProfileContractValidV053j(
    ...
    const bool physical = view.physical_transfer.channel !=
        STELLAR_PHYSICAL_CHANNEL_OPTICAL_V071;
    if(view.render_program == STELLAR_RENDER_PROGRAM_INVALID_V080)
    ...
    if(view.render_program ==
       ...
       (view.physical_transfer.channel !=
        ...
        !stellarHeroProjectionCalibrationValidV080(view.hero_calibration) ||
        !stellarHeroProjectionCalibrationSha256ValidV080(
            view.hero_calibration_sha256)))
      throw std::runtime_error("Invalid hero v080 projection contract: " + line);
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053j schema.");
  if(views.empty())
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053j.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053j supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++)
      if(views[viewNumber].render_program ==
        ...
        throw std::runtime_error(
            "v080 rejects legacy absolute rayMaxT payloads; re-export with "
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      scene.hero_calibration = view.hero_calibration;
      ...
      std::vector<float> materialColumns;
      std::vector<float> hotMaterialColumns;
      std::vector<float> outflowColumnFluxes;
      if(view.render_program ==
         STELLAR_RENDER_PROGRAM_HERO_PROJECTED_COPPER_BLUE_V080) {
        materialColumns.reserve(gpuResults.size());
        hotMaterialColumns.reserve(gpuResults.size());
        outflowColumnFluxes.reserve(gpuResults.size());
      }
      bool viewValid = true;
      for(size_t i = 0; i < gpuResults.size(); i++) {
        ...
        if(view.render_program ==
           STELLAR_RENDER_PROGRAM_HERO_PROJECTED_COPPER_BLUE_V080) {
          materialColumns.push_back(gpuResults[i].material_column_g_cm2);
          hotMaterialColumns.push_back(
              gpuResults[i].hot_material_column_g_cm2);
          outflowColumnFluxes.push_back(
              gpuResults[i].outflow_column_flux_g_cm_s);
        }
        viewValid = viewValid &&
      ...
      const float materialColumnP50 = positiveQuantile(
          materialColumns, 0.50f);
      const float materialColumnP90 = positiveQuantile(
          materialColumns, 0.90f);
      const float materialColumnP95 = positiveQuantile(
          materialColumns, 0.95f);
      const float hotMaterialColumnP90 = positiveQuantile(
          hotMaterialColumns, 0.90f);
      const float outflowColumnFluxP50 = positiveQuantile(
          outflowColumnFluxes, 0.50f);
      const float outflowColumnFluxP90 = positiveQuantile(
          outflowColumnFluxes, 0.90f);
      const float outflowColumnFluxP95 = positiveQuantile(
          outflowColumnFluxes, 0.95f);
      ...
      report << "renderer_version=stellar_v053j\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053j\n";
      ...
      report << "projection_material_column_reference_g_cm2="
             << view.hero_calibration.material_column_reference_g_cm2 << "\n";
      report << "projection_material_display_gain="
             << view.hero_calibration.material_display_gain
             << "\n";
      report << "projection_outflow_column_flux_reference_g_cm_s="
             << view.hero_calibration.outflow_column_flux_reference_g_cm_s
             << "\n";
      report << "projection_outflow_display_gain="
             << view.hero_calibration.outflow_display_gain << "\n";
      report << "projection_calibration_sha256="
             << view.hero_calibration_sha256 << "\n";
      report << "projection_material_column_p50_g_cm2="
             << materialColumnP50 << "\n";
      report << "projection_material_column_p90_g_cm2="
             << materialColumnP90 << "\n";
      report << "projection_material_column_p95_g_cm2="
             << materialColumnP95 << "\n";
      report << "projection_hot_material_column_p90_g_cm2="
             << hotMaterialColumnP90 << "\n";
      report << "projection_outflow_column_flux_p50_g_cm_s="
             << outflowColumnFluxP50 << "\n";
      report << "projection_outflow_column_flux_p90_g_cm_s="
             << outflowColumnFluxP90 << "\n";
      report << "projection_outflow_column_flux_p95_g_cm_s="
             << outflowColumnFluxP95 << "\n";
      report << "physical_color_aggregation="
             << (view.render_program ==
                     STELLAR_RENDER_PROGRAM_HERO_PROJECTED_COPPER_BLUE_V080 ?
                 "post_ray_material_rotation_hot_outflow_projection" :
      ...
      report << "physical_support="
             << (view.render_program ==
                     STELLAR_RENDER_PROGRAM_HERO_PROJECTED_COPPER_BLUE_V080 ?
                 "reviewed_rotation_range_union_hot_core_plus_outflow_flux" :
                 (stellarPhysicalOpticalUsesCompositeMomentsV078(
                     view.physical_optical.profile) ?
    ...
    summary << "renderer_version=stellar_v053j\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053j\n";
    ...
    summary << "render_program_contract=v080_retained_v078_or_projected_copper_blue\n";
    summary << "hero_components=material_column,rotation_moment,hot_material,outward_mass_flux\n";
