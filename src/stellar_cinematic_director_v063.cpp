...
double visualRelativeCorrection(
    const StellarDirectorVisualCalibrationRowV063 &row)
{
    ...
    VisualScaleSample *sample)
{
  if(!sample)
    return false;
  std::vector<const StellarDirectorVisualCalibrationRowV063 *> rows;
  ...
  const StellarDirectorVisualCalibrationRowV063 *left = rows.front();
  const StellarDirectorVisualCalibrationRowV063 *right = rows.front();
...
bool poseForShot(const StellarCameraState &state,
                 const StellarDirectorShotV063 &shot,
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    StellarDirectorLandmarkRowV063 row = {};
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_cinematic_shots_v063", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    ...
    if(!shots->empty()) {
      const StellarDirectorShotV063 &previous = shots->back();
  ...
  if(!schema_seen)
    return fail(error, "Missing schema=stellar_cinematic_shots_v063.");
  if(shots->empty())
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_visual_framing_calibration_v063", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    ...
    if(value.size() != 8)
      return fail(error, "Visual calibration row must have 8 columns at line " +
                  std::to_string(line_number));
    StellarDirectorVisualCalibrationRowV063 row = {};
  ...
  if(!schema_seen)
    return fail(error,
                "Missing schema=stellar_visual_framing_calibration_v063.");
    ...
    StellarDirectorResultV063 *result, std::string *error)
{
  if(!result || landmarks.empty() || shots.empty() || calibration.empty() ||
     ...
     !stellarDirectorValidMotionBudgetV063(budget))
  ...
  for(std::size_t index = 0; index < landmarks.size(); ++index) {
    ...
    StellarDirectorPathRowV063 row = {};
  ...
  for(std::size_t index = 0; index < result->path.size(); ++index) {
    const StellarDirectorPathRowV063 &row = result->path[index];
    StellarDirectorDiagnosticRowV063 diagnostic = {};
    ...
    if(index > 0) {
      const StellarDirectorPathRowV063 &previous = result->path[index - 1];
...
StellarDirectorMotionAssessmentV063 stellarDirectorAssessMotionV063(
    ...
    const StellarDirectorMotionBudgetV063 &budget)
{
  StellarDirectorMotionAssessmentV063 assessment = {};
  assessment.passed = stellarDirectorValidMotionBudgetV063(budget);
    ...
    const std::string &calibration_path)
{
  ...
  output << "# schema=stellar_camera_path_v055\n"
         << "# director_schema=stellar_cinematic_director_v063\n"
...
{
  std::ostringstream output;
  output << "# schema=stellar_camera_diagnostics_v063\n"
  ...
  for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
    ...
    output << row.snapshot << " " << row.time_seconds << " " << row.shot_name
           ...
           << " " << row.scale_limited
           << " " << row.visual_calibration_applied
           << " " << stellarDirectorVisualMetricNameV063(row.visual_metric)
    ...
    bool dry_run)
{
  std::ostringstream output;
  output << std::setprecision(17)
         << "schema=stellar_cinematic_direction_manifest_v063\n"
         ...
         << "framing.visual_calibration.schema="
            "stellar_visual_framing_calibration_v063\n"
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    output << "shot." << index << ".name=" << shot.name << "\n"
           ...
           << stellarDirectorExtentSourceNameV063(shot.extent_source) << "\n"
           << "shot." << index << ".screen_half_extent_override_cm="
           << (shot.has_scale_override ?
               shot.screen_half_extent_override_cm : 0.0) << "\n"
           << "shot." << index << ".transition_kind="
           << stellarDirectorTransitionKindNameV063(shot.transition_kind)
           << "\n"
           ...
           << "shot." << index << ".easing="
           << (shot.transition_kind == STELLAR_DIRECTOR_TRANSITION_CONTINUOUS ?
               stellarDirectorEasingNameV063(shot.easing) : "-") << "\n";
  }
  for(std::size_t index = 0; index < calibration.size(); ++index) {
    const StellarDirectorVisualCalibrationRowV063 &row = calibration[index];
    output << "visual_calibration." << index << ".snapshot="
           << row.snapshot << "\n"
           << "visual_calibration." << index << ".shot="
           << row.shot_name << "\n"
           << "visual_calibration." << index << ".metric="
           << stellarDirectorVisualMetricNameV063(row.metric) << "\n"
