...
void setMotionBudgetOption(const std::string &name, const std::string &text,
                           StellarDirectorMotionBudgetV062 *budget)
{
  const double value = parseFinite(text, name);
...
int main(int argc, char **argv)
{
  try {
    ...
    StellarDirectorMotionBudgetV062 motion_budget = {0.25, 1.0, 5.0};
    ...
    if(!stellarDirectorValidMotionBudgetV062(motion_budget))
    ...
    std::string error;
    if(!stellarDirectorLoadLandmarksV062(landmarks_input, &landmarks, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadShotsV062(shots_input, &shots, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadVisualCalibrationV062(
           calibration_input, &calibration, &error))
      throw std::runtime_error(error);
    StellarDirectorResultV062 result;
    if(!stellarDirectorBuildV062(landmarks, shots, calibration, filter,
                                 motion_budget,
                                 &result, &error))
      throw std::runtime_error(error);
    const StellarDirectorMotionAssessmentV062 motion_assessment =
        stellarDirectorAssessMotionV062(result, motion_budget);
    ...
    std::string manifest = stellarDirectorManifestV062(
    ...
    if(path_given && motion_assessment.passed)
      writeNoClobber(path_path, stellarDirectorPathTableV062(
          result, landmarks_path, shots_path, calibration_path),
          "camera path");
    writeNoClobber(diagnostics_path,
                   stellarDirectorDiagnosticsTableV062(result),
                   "diagnostics");
    writeNoClobber(preview_path, stellarDirectorPreviewSvgV062(result),
                   "preview");
    writeNoClobber(manifest_path, manifest, "manifest");
    if(path_given && !motion_assessment.passed) {
      std::cerr << "STELLAR_CAMERA_DIRECTOR_V062_MOTION_REJECTED roll_deg_per_frame="
    ...
    std::cout << "STELLAR_CAMERA_DIRECTOR_V062_OK rows="
              << result.path.size() << " shots=" << shots.size()
              << " visual_calibration_rows=" << calibration.size()
              << " dry_run=" << (dry_run ? "true" : "false") << "\n";
    return 0;
  } catch(const std::exception &error) {
    std::cerr << "STELLAR_CAMERA_DIRECTOR_V062_ERROR " << error.what()
              << "\n";
    return 1;
  }
}
