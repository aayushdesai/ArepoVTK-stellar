...
void setMotionBudgetOption(const std::string &name, const std::string &text,
                           StellarDirectorMotionBudgetV059 *budget)
{
  const double value = parseFinite(text, name);
...
int main(int argc, char **argv)
{
  try {
    ...
    StellarDirectorMotionBudgetV059 motion_budget = {0.25, 1.0};
    ...
    if(!stellarDirectorValidMotionBudgetV059(motion_budget))
    ...
    std::string error;
    if(!stellarDirectorLoadLandmarksV059(landmarks_input, &landmarks, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadShotsV059(shots_input, &shots, &error))
      throw std::runtime_error(error);
    StellarDirectorResultV059 result;
    if(!stellarDirectorBuildV059(landmarks, shots, filter, &result, &error))
      throw std::runtime_error(error);
    const StellarDirectorMotionAssessmentV059 motion_assessment =
        stellarDirectorAssessMotionV059(result, motion_budget);
    ...
    std::string manifest = stellarDirectorManifestV059(
    ...
    if(path_given && motion_assessment.passed)
      writeNoClobber(path_path, stellarDirectorPathTableV059(
          result, landmarks_path, shots_path), "camera path");
    writeNoClobber(diagnostics_path,
                   stellarDirectorDiagnosticsTableV059(result),
                   "diagnostics");
    writeNoClobber(preview_path, stellarDirectorPreviewSvgV059(result),
                   "preview");
    writeNoClobber(manifest_path, manifest, "manifest");
    if(path_given && !motion_assessment.passed) {
      std::cerr << "STELLAR_CAMERA_DIRECTOR_V059_MOTION_REJECTED roll_deg_per_frame="
    ...
    std::cout << "STELLAR_CAMERA_DIRECTOR_V059_OK rows="
              << result.path.size() << " shots=" << shots.size()
              << " dry_run=" << (dry_run ? "true" : "false") << "\n";
    return 0;
  } catch(const std::exception &error) {
    std::cerr << "STELLAR_CAMERA_DIRECTOR_V059_ERROR " << error.what()
              << "\n";
    return 1;
  }
}
