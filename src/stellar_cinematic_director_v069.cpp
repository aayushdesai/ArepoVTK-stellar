...
double visualRelativeCorrection(
    const StellarDirectorVisualCalibrationRowV069 &row)
{
  const double ratio = row.observed_fill / row.target_fill;
  return visualMetricIsArea(row.metric) ? std::sqrt(ratio) : ratio;
}

double visualAbsoluteTargetHalfExtent(
    const StellarDirectorVisualCalibrationRowV069 &row)
{
  return row.source_screen_half_extent_cm * visualRelativeCorrection(row);
}
...
struct VisualScaleSample {
  ...
  double absolute_target_half_extent_cm;
};
    ...
    VisualScaleSample *sample)
{
  if(!sample)
    return false;
  std::vector<const StellarDirectorVisualCalibrationRowV069 *> rows;
  ...
  const StellarDirectorVisualCalibrationRowV069 *left = rows.front();
  const StellarDirectorVisualCalibrationRowV069 *right = rows.front();
  ...
  sample->source_screen_half_extent_cm = std::exp(
      (1.0 - fraction) * std::log(left->source_screen_half_extent_cm) +
      fraction * std::log(right->source_screen_half_extent_cm));
  sample->absolute_target_half_extent_cm = std::exp(
      (1.0 - fraction) * std::log(visualAbsoluteTargetHalfExtent(*left)) +
      fraction * std::log(visualAbsoluteTargetHalfExtent(*right)));
  return std::isfinite(sample->source_screen_half_extent_cm) &&
      sample->source_screen_half_extent_cm > 0.0 &&
      std::isfinite(sample->absolute_target_half_extent_cm) &&
      sample->absolute_target_half_extent_cm > 0.0 &&
...
bool poseForShot(const StellarCameraState &state,
                 const StellarDirectorShotV069 &shot,
                 ...
                 StellarCameraPose *pose)
{
  ...
  if(!stellarCameraNormalize(pose->up))
    return false;
  double right[3];
  stellarCameraCross(pose->up, view_offset, right);
  if(!stellarCameraNormalize(right))
    return false;
  const double roll = shot.roll_radians * progress;
  for(int component = 0; component < 3; ++component)
    pose->up[component] = std::cos(roll) * pose->up[component] +
        std::sin(roll) * right[component];
...
struct SignedFramingSampleV069 {
  bool applied;
  std::string name;
  double fraction;
  double shift_x_cm;
...
bool sampleSignedFraming(
    const std::vector<StellarDirectorLandmarkRowV069> &landmarks,
    const std::vector<StellarDirectorSignedFramingRowV069> &framing,
    unsigned long long snapshot, double time_seconds,
    double base_target_half_extent_cm, SignedFramingSampleV069 *sample)
{
  if(!sample || !(base_target_half_extent_cm > 0.0))
    return false;
  sample->applied = false;
  sample->name = "identity";
  sample->fraction = 0.0;
  sample->shift_x_cm = 0.0;
  sample->shift_y_cm = 0.0;
  sample->absolute_target_half_extent_cm = base_target_half_extent_cm;
  if(snapshot < framing.front().transition_start_snapshot)
    return true;

  std::size_t active = 0;
  while(active + 1 < framing.size() &&
        snapshot >= framing[active + 1].transition_start_snapshot)
    ++active;
  const StellarDirectorSignedFramingRowV069 &target = framing[active];
  const double target_x = target.source_screen_half_extent_cm *
      target.look_at_shift_screen_x_fraction;
  const double target_y = target.source_screen_half_extent_cm *
      target.look_at_shift_screen_y_fraction;
  const double target_scale = target.source_screen_half_extent_cm *
      target.half_extent_scale_factor;
  double left_x = 0.0;
  double left_y = 0.0;
  double left_scale = base_target_half_extent_cm;
  if(active > 0) {
    const StellarDirectorSignedFramingRowV069 &previous = framing[active - 1];
    left_x = previous.source_screen_half_extent_cm *
        previous.look_at_shift_screen_x_fraction;
    left_y = previous.source_screen_half_extent_cm *
        previous.look_at_shift_screen_y_fraction;
    left_scale = previous.source_screen_half_extent_cm *
        previous.half_extent_scale_factor;
  }
  double fraction = 1.0;
  if(snapshot <= target.transition_end_snapshot) {
    const std::size_t start_index = findLandmark(
        landmarks, target.transition_start_snapshot);
    const std::size_t end_index = findLandmark(
        landmarks, target.transition_end_snapshot);
    if(start_index == landmarks.size() || end_index == landmarks.size())
      return false;
    const double start_time = landmarks[start_index].landmark.time_seconds;
    const double end_time = landmarks[end_index].landmark.time_seconds;
    if(!(end_time > start_time))
      return false;
    fraction = ease((time_seconds - start_time) / (end_time - start_time),
                    target.easing);
  }
  sample->applied = true;
  sample->name = target.name;
  sample->fraction = fraction;
  sample->shift_x_cm = (1.0 - fraction) * left_x + fraction * target_x;
  sample->shift_y_cm = (1.0 - fraction) * left_y + fraction * target_y;
  sample->absolute_target_half_extent_cm = std::exp(
      (1.0 - fraction) * std::log(left_scale) +
      fraction * std::log(target_scale));
  return std::isfinite(sample->shift_x_cm) &&
      std::isfinite(sample->shift_y_cm) &&
      std::isfinite(sample->absolute_target_half_extent_cm) &&
      sample->absolute_target_half_extent_cm > 0.0;
}

bool applySignedFraming(const SignedFramingSampleV069 &sample,
                        StellarCameraPose *pose)
{
  if(!pose || !sample.applied)
    return pose != 0;
  double view[3];
  for(int component = 0; component < 3; ++component)
    view[component] = pose->position[component] - pose->look_at[component];
  if(!stellarCameraNormalize(view))
    return false;
  double up[3] = {pose->up[0], pose->up[1], pose->up[2]};
  if(!stellarCameraNormalize(up))
    return false;
  double right[3];
  stellarCameraCross(up, view, right);
  if(!stellarCameraNormalize(right))
    return false;
  for(int component = 0; component < 3; ++component) {
    const double shift = sample.shift_x_cm * right[component] +
        sample.shift_y_cm * up[component];
    pose->look_at[component] += shift;
    pose->position[component] += shift;
  }
  const double scale_factor = sample.absolute_target_half_extent_cm /
      pose->screen_half_extent_cm;
  return applyVisualScale(scale_factor, pose);
}
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    StellarDirectorLandmarkRowV069 row = {};
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_cinematic_shots_v069", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    const std::vector<std::string> value = tokens(line.substr(start));
    if(value.size() != 17)
      return fail(error, "Shot row must have 17 columns at line " +
                  std::to_string(line_number));
    StellarDirectorShotV069 shot = {};
    shot.name = value[0];
    shot.subject = value[4];
    shot.mode = parseMode(value[3]);
    shot.extent_source = parseExtentSource(value[12]);
    shot.transition_kind = parseTransitionKind(value[14]);
    shot.easing = value[16] == "-" ? -1 : parseEasing(value[16]);
    ...
    if(!safeToken(shot.name) || !safeToken(shot.subject) || shot.mode < 0 ||
       ...
       !parseFinite(value[7], &roll_degrees) ||
       !parseFinite(value[8], &shot.framing_margin) ||
       !parseFinite(value[11], &lobe_sign))
      return fail(error, "Invalid shot value at line " +
                  std::to_string(line_number));
    shot.has_orbit_degrees = value[9] != "-";
    shot.has_orbit_period = value[10] != "-";
    shot.has_scale_override = value[13] != "-";
    if((shot.has_orbit_degrees && !parseFinite(value[9], &orbit_degrees)) ||
       (shot.has_orbit_period &&
        !parseFinite(value[10], &shot.orbit_period_seconds)) ||
       (shot.has_scale_override &&
        !parseFinite(value[13], &shot.screen_half_extent_override_cm)) ||
       (shot.transition_kind == STELLAR_DIRECTOR_TRANSITION_CONTINUOUS &&
        !parseUnsigned(value[15], &shot.transition_end_snapshot)))
    ...
    if(shot.has_orbit_degrees && shot.has_orbit_period)
      return fail(error, "Use orbit degrees or period, not both, at line " +
                  std::to_string(line_number));
    if(shot.transition_kind == STELLAR_DIRECTOR_TRANSITION_CUT)
      return fail(error, "V069 requires continuous camera motion; cuts are not allowed at line " +
                  std::to_string(line_number));
    ...
    shot.roll_radians = roll_degrees * kPi / 180.0;
    ...
    if(!shots->empty()) {
      const StellarDirectorShotV069 &previous = shots->back();
  ...
  if(!schema_seen)
    return fail(error, "Missing schema=stellar_cinematic_shots_v069.");
  if(shots->empty())
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_visual_framing_calibration_v069", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    const std::vector<std::string> value = tokens(line.substr(start));
    if(value.size() != 8)
      return fail(error, "Visual calibration row must have 8 columns at line " +
                  std::to_string(line_number));
    StellarDirectorVisualCalibrationRowV069 row = {};
    row.shot_name = value[1];
    row.metric = parseVisualMetric(value[2]);
    row.source_frame_sha256 = value[6];
    row.source_camera_path_sha256 = value[7];
    if(!parseUnsigned(value[0], &row.snapshot) ||
       !safeToken(row.shot_name) || row.metric < 0 ||
       !parseFinite(value[3], &row.observed_fill) ||
       !parseFinite(value[4], &row.target_fill) ||
       !parseFinite(value[5], &row.source_screen_half_extent_cm) ||
       !validSha256(row.source_frame_sha256) ||
       !validSha256(row.source_camera_path_sha256))
    ...
    if(!(row.observed_fill > 0.0) || row.observed_fill > 1.0 ||
      ...
      return fail(error, "Visual fill values must be in (0,1] at line " +
                  std::to_string(line_number));
    if(!(row.source_screen_half_extent_cm > 0.0))
      return fail(error, "Visual source half extent must be positive at line " +
                  std::to_string(line_number));
  ...
  if(!schema_seen)
    return fail(error,
                "Missing schema=stellar_visual_framing_calibration_v069.");
...
{
  if(!framing)
    return fail(error, "Signed framing output is null.");
  framing->clear();
  bool schema_seen = false;
  std::string line;
  std::size_t line_number = 0;
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_signed_framing_application_v069", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    const std::vector<std::string> value = tokens(line.substr(start));
    if(value.size() != 20)
      return fail(error, "Signed framing row must have 20 columns at line " +
                  std::to_string(line_number));
    StellarDirectorSignedFramingRowV069 row = {};
    row.name = value[0];
    row.shot_name = value[1];
    row.easing = parseEasing(value[5]);
    row.feature_mode = value[6];
    row.features = value[7];
    row.framing_plan_sha256 = value[16];
    row.source_frame_sha256 = value[17];
    row.source_camera_path_sha256 = value[18];
    row.source_camera_row_sha256 = value[19];
    if(!safeToken(row.name) || !safeToken(row.shot_name) || row.easing < 0 ||
       ...
       !parseFinite(value[8], &row.minimum_weight) ||
       !parseFinite(value[9], &row.coverage) ||
       !parseFinite(value[10], &row.target_half_width_fraction) ||
       !parseFinite(value[11], &row.target_half_height_fraction) ||
       !parseFinite(value[12], &row.look_at_shift_screen_x_fraction) ||
       !parseFinite(value[13], &row.look_at_shift_screen_y_fraction) ||
       !parseFinite(value[14], &row.half_extent_scale_factor) ||
       !parseFinite(value[15], &row.source_screen_half_extent_cm) ||
       !validSha256(row.framing_plan_sha256) ||
       !validSha256(row.source_frame_sha256) ||
       !validSha256(row.source_camera_path_sha256) ||
       !validSha256(row.source_camera_row_sha256))
      return fail(error, "Invalid signed framing value at line " +
                  std::to_string(line_number));
    if(row.transition_start_snapshot >= row.transition_end_snapshot ||
       !(row.minimum_weight > 0.0) || row.minimum_weight > 1.0 ||
       (row.coverage != 0.90 && row.coverage != 0.98) ||
       !(row.target_half_width_fraction > 0.0) ||
       row.target_half_width_fraction > 1.0 ||
       !(row.target_half_height_fraction > 0.0) ||
       row.target_half_height_fraction > 1.0 ||
       !(row.half_extent_scale_factor > 0.0) ||
    ...
    if(row.feature_mode == "bipolar_union" &&
      ...
      return fail(error, "Bipolar signed framing must preserve both signed lobes at line " +
                  std::to_string(line_number));
    if(!framing->empty()) {
      const StellarDirectorSignedFramingRowV069 &previous = framing->back();
      if(row.transition_start_snapshot <= previous.transition_end_snapshot)
        return fail(error, "Signed framing windows must be ordered and nonoverlapping at line " +
                    std::to_string(line_number));
      for(std::size_t index = 0; index < framing->size(); ++index)
        if(framing->at(index).name == row.name)
          return fail(error, "Signed framing names must be unique at line " +
                      std::to_string(line_number));
    }
    framing->push_back(row);
  }
  if(!schema_seen)
    return fail(error,
                "Missing schema=stellar_signed_framing_application_v069.");
  if(framing->empty())
    return fail(error, "Signed framing table is empty.");
  if(error)
    error->clear();
  return true;
}
    ...
    StellarDirectorResultV069 *result, std::string *error)
{
  if(!result || landmarks.empty() || shots.empty() || calibration.empty() ||
     ...
     !stellarDirectorValidMotionBudgetV069(budget))
  ...
  for(std::size_t framing_index = 0; framing_index < framing.size();
      ++framing_index) {
    const StellarDirectorSignedFramingRowV069 &row = framing[framing_index];
    if(findLandmark(landmarks, row.source_snapshot) == landmarks.size() ||
       findLandmark(landmarks, row.transition_start_snapshot) ==
           landmarks.size() ||
       findLandmark(landmarks, row.transition_end_snapshot) == landmarks.size())
      return fail(error,
                  "Every signed framing source and window boundary must have a landmark row.");
    bool shot_found = false;
    for(std::size_t shot_index = 0; shot_index < shots.size(); ++shot_index)
      if(shots[shot_index].name == row.shot_name &&
         row.transition_start_snapshot >= shots[shot_index].start_snapshot &&
         row.transition_end_snapshot <= shots[shot_index].end_snapshot)
        shot_found = true;
    if(!shot_found)
      return fail(error,
                  "Every signed framing window must bind one containing shot.");
  }
  ...
  for(std::size_t index = 0; index < landmarks.size(); ++index) {
    ...
    StellarDirectorPathRowV069 row = {};
    ...
    double visual_absolute_target_half_extent_cm =
        visual.absolute_target_half_extent_cm;
    double visual_source_screen_half_extent_cm =
        visual.source_screen_half_extent_cm;
    double visual_target_fill = visual.target_fill;
    if(shot_index > 0 &&
      ...
      visual_absolute_target_half_extent_cm = std::exp(
          (1.0 - row.transition_fraction) *
              std::log(previous_visual.absolute_target_half_extent_cm) +
          row.transition_fraction *
              std::log(visual.absolute_target_half_extent_cm));
      visual_source_screen_half_extent_cm = std::exp(
          (1.0 - row.transition_fraction) *
              std::log(previous_visual.source_screen_half_extent_cm) +
          row.transition_fraction *
              std::log(visual.source_screen_half_extent_cm));
      visual_target_fill =
          (1.0 - row.transition_fraction) * previous_visual.target_fill +
          row.transition_fraction * visual.target_fill;
    }
    const double visual_scale_factor =
        visual_absolute_target_half_extent_cm /
    ...
    row.visual_target_fill = visual_target_fill;
    row.visual_source_screen_half_extent_cm =
        visual_source_screen_half_extent_cm;
    row.visual_absolute_target_half_extent_cm =
        visual_absolute_target_half_extent_cm;
    SignedFramingSampleV069 signed_sample = {};
    if(!sampleSignedFraming(landmarks, framing, row.snapshot, row.time_seconds,
                            row.pose.screen_half_extent_cm, &signed_sample) ||
       !applySignedFraming(signed_sample, &row.pose))
      return fail(error, "Signed framing application failed.");
    row.signed_framing_applied = signed_sample.applied ? 1 : 0;
    row.signed_framing_name = signed_sample.name;
    row.signed_framing_fraction = signed_sample.fraction;
    row.signed_absolute_target_half_extent_cm =
        signed_sample.absolute_target_half_extent_cm;
    row.signed_pan_x_fraction = signed_sample.shift_x_cm /
        row.pose.screen_half_extent_cm;
    row.signed_pan_y_fraction = signed_sample.shift_y_cm /
        row.pose.screen_half_extent_cm;
  ...
  double previous_artistic_pan_step = 0.0;
  std::size_t motion_segment_row = 0;
  for(std::size_t index = 0; index < result->path.size(); ++index) {
    const StellarDirectorPathRowV069 &row = result->path[index];
    StellarDirectorDiagnosticRowV069 diagnostic = {};
    ...
    diagnostic.visual_source_screen_half_extent_cm =
        row.visual_source_screen_half_extent_cm;
    diagnostic.visual_absolute_target_half_extent_cm =
        row.visual_absolute_target_half_extent_cm;
    diagnostic.signed_framing_applied = row.signed_framing_applied;
    diagnostic.signed_framing_name = row.signed_framing_name;
    diagnostic.signed_framing_fraction = row.signed_framing_fraction;
    diagnostic.signed_pan_x_fraction = row.signed_pan_x_fraction;
    diagnostic.signed_pan_y_fraction = row.signed_pan_y_fraction;
    diagnostic.signed_absolute_target_half_extent_cm =
        row.signed_absolute_target_half_extent_cm;
    if(row.cut_from_previous) {
      ...
      previous_artistic_pan_step = 0.0;
      motion_segment_row = 0;
    }
    if(index > 0) {
      const StellarDirectorPathRowV069 &previous = result->path[index - 1];
    ...
    if(index > 0 && !row.cut_from_previous) {
      ...
      const double pan_dx = row.signed_pan_x_fraction -
          previous.signed_pan_x_fraction;
      const double pan_dy = row.signed_pan_y_fraction -
          previous.signed_pan_y_fraction;
      diagnostic.artistic_pan_step_fraction_per_frame =
          std::sqrt(pan_dx * pan_dx + pan_dy * pan_dy);
      if(motion_segment_row > 1)
        diagnostic.artistic_pan_acceleration_fraction_per_frame2 =
            diagnostic.artistic_pan_step_fraction_per_frame -
            previous_artistic_pan_step;
      previous_artistic_pan_step =
          diagnostic.artistic_pan_step_fraction_per_frame;
    }
    ...
    const StellarDirectorMotionBudgetV069 &budget)
{
  return std::isfinite(budget.max_roll_deg_per_frame) &&
      ...
      std::isfinite(budget.max_artistic_pan_fraction_per_frame) &&
      ...
      budget.max_artistic_pan_fraction_per_frame > 0.0 &&
...
StellarDirectorMotionAssessmentV069 stellarDirectorAssessMotionV069(
    ...
    const StellarDirectorMotionBudgetV069 &budget)
{
  StellarDirectorMotionAssessmentV069 assessment = {};
  assessment.passed = stellarDirectorValidMotionBudgetV069(budget);
  for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
    ...
    if(row.artistic_pan_step_fraction_per_frame >
       assessment.maximum_artistic_pan_fraction_per_frame) {
      assessment.maximum_artistic_pan_fraction_per_frame =
          row.artistic_pan_step_fraction_per_frame;
      assessment.maximum_artistic_pan_snapshot = row.snapshot;
    }
  ...
  assessment.passed = assessment.passed &&
      ...
      withinBudget(assessment.maximum_artistic_pan_fraction_per_frame,
                   budget.max_artistic_pan_fraction_per_frame) &&
    ...
    const std::string &framing_path)
{
  ...
  output << "# schema=stellar_camera_path_v055\n"
         << "# director_schema=stellar_cinematic_director_v069\n"
         << "# landmarks=" << landmarks_path << "\n"
         << "# shots=" << shots_path << "\n"
         << "# visual_calibration=" << calibration_path << "\n"
         << "# signed_framing=" << framing_path << "\n"
...
{
  std::ostringstream output;
  output << "# schema=stellar_camera_diagnostics_v069\n"
         << "# snapshot time shot transition_from transition_to "
            ...
            "visual_source_screen_half_extent_cm "
            "visual_absolute_target_half_extent_cm "
            "signed_framing_applied signed_framing_name "
            "signed_framing_fraction signed_pan_x_fraction "
            "signed_pan_y_fraction artistic_pan_step_fraction_per_frame "
            "artistic_pan_acceleration_fraction_per_frame2 "
            "signed_absolute_target_half_extent_cm "
  ...
  for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
    ...
    output << row.snapshot << " " << row.time_seconds << " " << row.shot_name
           ...
           << " " << row.scale_limited
           << " " << row.visual_calibration_applied
           << " " << stellarDirectorVisualMetricNameV069(row.visual_metric)
           ...
           << " " << row.visual_source_screen_half_extent_cm
           << " " << row.visual_absolute_target_half_extent_cm
           << " " << row.signed_framing_applied
           << " " << row.signed_framing_name
           << " " << row.signed_framing_fraction
           << " " << row.signed_pan_x_fraction
           << " " << row.signed_pan_y_fraction
           << " " << row.artistic_pan_step_fraction_per_frame
           << " " << row.artistic_pan_acceleration_fraction_per_frame2
           << " " << row.signed_absolute_target_half_extent_cm
    ...
    bool dry_run)
{
  std::ostringstream output;
  output << std::setprecision(17)
         << "schema=stellar_cinematic_direction_manifest_v069\n"
         << "landmarks_path=" << landmarks_path << "\n"
         << "shots_path=" << shots_path << "\n"
         << "visual_calibration_path=" << calibration_path << "\n"
         << "signed_framing_path=" << framing_path << "\n"
         ...
         << "framing.visual_calibration.schema="
            "stellar_visual_framing_calibration_v069\n"
         << "framing.visual_calibration.interpolation="
            "per_shot_log_scale_piecewise_linear_in_landmark_time\n"
         << "framing.visual_calibration.area_rule="
            "source_half_extent*sqrt(observed/target)\n"
         << "framing.visual_calibration.bbox_rule="
            "source_half_extent*observed/target\n"
         << "framing.visual_calibration.absolute_target_rebased=true\n"
         << "framing.visual_calibration.source_frame_hash_bound=true\n"
         << "framing.visual_calibration.source_camera_path_hash_bound=true\n"
         << "framing.signed.schema=stellar_signed_framing_application_v069\n"
         << "framing.signed.plan_hash_bound=true\n"
         << "framing.signed.source_frame_hash_bound=true\n"
         << "framing.signed.source_camera_path_hash_bound=true\n"
         << "framing.signed.source_camera_row_hash_bound=true\n"
         << "framing.signed.shift_semantics=source_screen_basis_physical_cm\n"
         << "framing.signed.scale_semantics=source_half_extent_times_v067_scale\n"
         << "transitions.declared_kinds=continuous_only\n"
         << "transitions.hard_cuts_allowed=false\n";
  output << "motion_budget.max_roll_deg_per_frame="
         << budget.max_roll_deg_per_frame << "\n"
         << "motion_budget.max_zoom_percent_per_frame="
         << budget.max_zoom_percent_per_frame << "\n"
         << "motion_budget.max_artistic_pan_fraction_per_frame="
         << budget.max_artistic_pan_fraction_per_frame << "\n"
         ...
         << "motion.maximum_artistic_pan_fraction_per_frame="
         << assessment.maximum_artistic_pan_fraction_per_frame << "\n"
         << "motion.maximum_artistic_pan_snapshot="
         << assessment.maximum_artistic_pan_snapshot << "\n"
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    output << "shot." << index << ".name=" << shot.name << "\n"
           ...
           << stellarDirectorExtentSourceNameV069(shot.extent_source) << "\n"
           << "shot." << index << ".screen_half_extent_override_cm="
           << (shot.has_scale_override ?
               shot.screen_half_extent_override_cm : 0.0) << "\n"
           << "shot." << index << ".transition_kind="
           << stellarDirectorTransitionKindNameV069(shot.transition_kind)
           << "\n"
           ...
           << "shot." << index << ".easing="
           << (shot.transition_kind == STELLAR_DIRECTOR_TRANSITION_CONTINUOUS ?
               stellarDirectorEasingNameV069(shot.easing) : "-") << "\n";
  }
  for(std::size_t index = 0; index < calibration.size(); ++index) {
    const StellarDirectorVisualCalibrationRowV069 &row = calibration[index];
    output << "visual_calibration." << index << ".snapshot="
           << row.snapshot << "\n"
           << "visual_calibration." << index << ".shot="
           << row.shot_name << "\n"
           << "visual_calibration." << index << ".metric="
           << stellarDirectorVisualMetricNameV069(row.metric) << "\n"
           << "visual_calibration." << index << ".observed_fill="
           << row.observed_fill << "\n"
           << "visual_calibration." << index << ".target_fill="
           << row.target_fill << "\n"
           << "visual_calibration." << index
           << ".source_screen_half_extent_cm="
           << row.source_screen_half_extent_cm << "\n"
           << "visual_calibration." << index
           << ".relative_correction_factor="
           << visualRelativeCorrection(row) << "\n"
           << "visual_calibration." << index
           << ".absolute_target_half_extent_cm="
           << visualAbsoluteTargetHalfExtent(row) << "\n"
           << "visual_calibration." << index << ".source_frame_sha256="
           << row.source_frame_sha256 << "\n"
           << "visual_calibration." << index
           << ".source_camera_path_sha256="
           << row.source_camera_path_sha256 << "\n";
  }
  for(std::size_t index = 0; index < framing.size(); ++index) {
    const StellarDirectorSignedFramingRowV069 &row = framing[index];
    output << "signed_framing." << index << ".name=" << row.name << "\n"
           << "signed_framing." << index << ".shot=" << row.shot_name << "\n"
           << "signed_framing." << index << ".source_snapshot="
           << row.source_snapshot << "\n"
           << "signed_framing." << index << ".transition_window="
           << row.transition_start_snapshot << ":"
           << row.transition_end_snapshot << "\n"
           << "signed_framing." << index << ".easing="
           << stellarDirectorEasingNameV069(row.easing) << "\n"
           << "signed_framing." << index << ".feature_mode="
           << row.feature_mode << "\n"
           << "signed_framing." << index << ".features="
           << row.features << "\n"
           << "signed_framing." << index << ".minimum_weight="
           << row.minimum_weight << "\n"
           << "signed_framing." << index << ".coverage="
           << row.coverage << "\n"
           << "signed_framing." << index << ".target_half_width_fraction="
           << row.target_half_width_fraction << "\n"
           << "signed_framing." << index << ".target_half_height_fraction="
           << row.target_half_height_fraction << "\n"
           << "signed_framing." << index << ".look_at_shift_screen_x_fraction="
           << row.look_at_shift_screen_x_fraction << "\n"
           << "signed_framing." << index << ".look_at_shift_screen_y_fraction="
           << row.look_at_shift_screen_y_fraction << "\n"
           << "signed_framing." << index << ".half_extent_scale_factor="
           << row.half_extent_scale_factor << "\n"
           << "signed_framing." << index << ".source_screen_half_extent_cm="
           << row.source_screen_half_extent_cm << "\n"
           << "signed_framing." << index << ".framing_plan_sha256="
           << row.framing_plan_sha256 << "\n"
           << "signed_framing." << index << ".source_frame_sha256="
           << row.source_frame_sha256 << "\n"
           << "signed_framing." << index << ".source_camera_path_sha256="
           << row.source_camera_path_sha256 << "\n"
           << "signed_framing." << index << ".source_camera_row_sha256="
           << row.source_camera_row_sha256 << "\n";
  }
  return output.str();
}
    ...
    const StellarDirectorResultV069 &result)
{
  ...
  for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
    ...
    maximum_motion = std::max(maximum_motion,
        std::max(row.axis_rate_rad_per_s,
                 std::max(std::abs(row.log_zoom_rate_per_s),
                          row.artistic_pan_step_fraction_per_frame)));
  }
  maximum_motion = std::max(maximum_motion, 1.0e-12);
  const char *motion_colors[] = {"#8b5e9b", "#d08a2e", "#247a73"};
  for(int series = 0; series < 3; ++series) {
    ...
    for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
      ...
      const double value = series == 0 ? row.axis_rate_rad_per_s :
          (series == 1 ? std::abs(row.log_zoom_rate_per_s) :
           row.artistic_pan_step_fraction_per_frame);
      const double x = left + plot_width *
          (static_cast<double>(row.snapshot) - first_snapshot) / span;
  ...
  svg << "<text x=\"90\" y=\"555\" class=\"small\">Blue raw material, red raw disk, "
         "green raw outflow occupancy; purple axis, amber zoom, teal authored pan.</text>\n"
