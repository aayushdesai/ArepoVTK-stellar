...
bool poseForShot(const StellarCameraState &state,
                 const StellarDirectorShotV056 &shot,
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    StellarDirectorLandmarkRowV056 row = {};
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_cinematic_shots_v056", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    const std::vector<std::string> value = tokens(line.substr(start));
    if(value.size() != 13)
      return fail(error, "Shot row must have 13 columns at line " +
                  std::to_string(line_number));
    StellarDirectorShotV056 shot = {};
    shot.name = value[0];
    shot.subject = value[4];
    shot.mode = parseMode(value[3]);
    shot.easing = parseEasing(value[12]);
    ...
    if(!safeToken(shot.name) || !safeToken(shot.subject) || shot.mode < 0 ||
       shot.easing < 0 || !parseUnsigned(value[1], &shot.start_snapshot) ||
       ...
       !parseFinite(value[11], &shot.transition_seconds))
    ...
    if((shot.has_orbit_degrees && !parseFinite(value[8], &orbit_degrees)) ||
      ...
      return fail(error, "Invalid orbit control at line " +
                  std::to_string(line_number));
    ...
    if(!shots->empty()) {
      const StellarDirectorShotV056 &previous = shots->back();
  ...
  if(!schema_seen)
    return fail(error, "Missing schema=stellar_cinematic_shots_v056.");
  if(shots->empty())
    return fail(error, "Shot table is empty.");
  if(shots->back().transition_seconds != 0.0)
    return fail(error, "The final shot transition must be zero.");
  if(error)
    error->clear();
  return true;
}
    ...
    StellarDirectorResultV056 *result, std::string *error)
{
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    const double duration = landmarks[shot_end[index]].landmark.time_seconds -
        landmarks[shot_start[index]].landmark.time_seconds;
    if(shots[index].transition_seconds > duration)
      return fail(error, "A shot transition exceeds its physical duration.");
  }
  ...
  for(std::size_t index = 0; index < landmarks.size(); ++index) {
    ...
    StellarDirectorPathRowV056 row = {};
    ...
    if(!poseForShot(state, shots[shot_index], start_time, end_time, &row.pose))
      return fail(error, "Directed camera pose failed.");

    if(shot_index + 1 < shots.size() &&
       shots[shot_index].transition_seconds > 0.0 &&
       row.time_seconds >= end_time - shots[shot_index].transition_seconds) {
      const double raw_fraction =
          (row.time_seconds - (end_time - shots[shot_index].transition_seconds)) /
          shots[shot_index].transition_seconds;
      row.transition_fraction = ease(raw_fraction, shots[shot_index].easing);
      row.transition_to = shots[shot_index + 1].name;
      StellarCameraPose next_pose = {};
      const double next_start =
          landmarks[shot_start[shot_index + 1]].landmark.time_seconds;
      const double next_end =
          landmarks[shot_end[shot_index + 1]].landmark.time_seconds;
      if(!poseForShot(state, shots[shot_index + 1], next_start, next_end,
                      &next_pose) ||
         !blendPoses(row.pose, next_pose, row.transition_fraction, &row.pose))
  ...
  for(std::size_t index = 0; index < result->path.size(); ++index) {
    const StellarDirectorPathRowV056 &row = result->path[index];
    StellarDirectorDiagnosticRowV056 diagnostic = {};
    ...
    if(index > 0) {
      const StellarDirectorPathRowV056 &previous = result->path[index - 1];
    ...
    const std::string &landmarks_path, const std::string &shots_path)
{
  ...
  output << "# schema=stellar_camera_path_v055\n"
         << "# director_schema=stellar_cinematic_director_v056\n"
...
{
  std::ostringstream output;
  output << "# schema=stellar_camera_diagnostics_v056\n"
         << "# snapshot time shot transition_to transition_fraction "
            "center_speed center_acceleration center_jerk camera_speed "
            "camera_acceleration camera_jerk axis_rate roll_rate "
            "log_zoom_rate log_zoom_acceleration material_occupancy "
            "disk_occupancy outflow_occupancy material_clipped disk_clipped "
            "outflow_clipped\n";
  output << std::setprecision(17);
    ...
    bool dry_run)
{
  std::ostringstream output;
  output << std::setprecision(17)
         << "schema=stellar_cinematic_direction_manifest_v056\n"
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    output << "shot." << index << ".name=" << shot.name << "\n"
           ...
           << "shot." << index << ".transition_seconds="
           << shot.transition_seconds << "\n"
           << "shot." << index << ".easing="
           << stellarDirectorEasingNameV056(shot.easing) << "\n";
  }
  return output.str();
}
    ...
    const StellarDirectorResultV056 &result)
{
  ...
  const char *panel_titles[] = {"Geometric occupancy", "Motion rates (normalized)"};
  ...
  for(int series = 0; series < 3; ++series) {
    ...
    for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
      const StellarDirectorDiagnosticRowV056 &row = result.diagnostics[index];
      const double value = series == 0 ? row.material_occupancy :
          (series == 1 ? row.disk_occupancy : row.outflow_occupancy);
  ...
  svg << "<text x=\"90\" y=\"555\" class=\"small\">Blue material, red disk, "
         "green outflow occupancy; purple axis rate, amber zoom rate.</text>\n"
