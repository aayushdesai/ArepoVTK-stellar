...
struct RayResult {
  float rgb[3];
  StellarHeroSurfaceSampleV081 hero_surface;
  uint32_t cells;
  uint32_t samples;
  uint32_t status;
};
...
struct DeviceScene {
  ...
  int render_program;
  StellarHeroDisplayContractV081 hero_display;
  float exposure;
  float black_point;
  float saturation;
                                        ...
                                        double *entry, double *exit)
{
  ...
  const bool entryRelative =
      (scene.scene_flags &
       AREPO_STELLAR_ENTRY_RELATIVE_RAY_LIMIT_V080) != 0;
  const StellarGpuRayDepthDecisionV080 depth =
      stellarResolveGpuRayDepthV080(
          t0, t1, entryRelative ? 0.0 : scene.ray_max_t,
          entryRelative ? scene.ray_max_t : 0.0);
  *entry = t0;
  *exit = depth.maximum_t;
  return depth.valid != 0;
}
...
__host__ __device__ void accumulateHeroProjection(
    const DeviceScene &scene, const double point[3], float density,
    float temperature, const float velocity[3], RayResult *result)
{
  if(scene.render_program !=
  ...
  const StellarHeroProjectionSampleV080 sample =
      stellarHeroProjectionSampleV080(
          physical, scene.physical_transfer, scene.physical_optical);
  stellarUpdateHeroSurfaceV081(&result->hero_surface, sample);
}
...
__host__ __device__ RayResult renderRay(const DeviceScene &scene, uint64_t rayIndex)
{
  ...
  result.hero_surface = stellarEmptyHeroSurfaceSampleV081();
  ...
  for(int iteration = 0; iteration < kMaxCellsPerRay && currentT < ray.t_max; iteration++) {
    ...
    if(length > 1.0e-11) {
      ...
      for(int sampleIndex = 0; sampleIndex < scene.samples_per_cell; sampleIndex++) {
        ...
        if(scene.render_program ==
           STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081) {
          accumulateHeroProjection(
              scene, point, density, temperature, velocity, &result);
        } else {
          const StellarOpticalSample optical = evaluateTransfer(
              scene, cell, point, density, temperature, velocity);
  ...
  if(scene.render_program ==
         STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 &&
     !stellarHeroSurfaceFiniteV081(result.hero_surface))
...
void writeTga(const std::string &filename, const std::vector<RayResult> &results,
              ...
              int renderProgram,
              const StellarPhysicalOpticalParametersV076 &physicalOptical,
              const StellarHeroDisplayContractV081 &heroDisplay,
              float materialReference, float outflowReference)
{
...
#pragma omp parallel for schedule(static)
  for(long long pixelIndex = 0; pixelIndex < static_cast<long long>(results.size()); pixelIndex++) {
    ...
    float mapped[3];
    const bool adaptiveHero = renderProgram ==
        STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081;
    bool heroSignal = false;
    if(adaptiveHero) {
      heroSignal =
          stellarHeroAdaptiveResponseV081(
              results[i].hero_surface.material_peak_density_g_cm3,
              materialReference,
              heroDisplay.response_floor_fraction,
              heroDisplay.material_display_gain) > 0.0f ||
          stellarHeroAdaptiveResponseV081(
              results[i].hero_surface.outflow_peak_flux_g_cm2_s,
              outflowReference,
              heroDisplay.response_floor_fraction,
              heroDisplay.outflow_display_gain) > 0.0f;
      stellarDecodeHeroSurfaceV081(
          results[i].hero_surface, materialReference, outflowReference,
          heroDisplay, mapped);
    } else {
      float linearRgb[3];
      stellarDecodeRenderProgramV080(
          renderProgram, results[i].rgb, physicalOptical, linearRgb);
      for(int channel = 0; channel < 3; channel++)
        mapped[channel] = stellarFilmicMap(
            std::max(0.0f, linearRgb[channel]), exposure, blackPoint);
    }
    if(!adaptiveHero || heroSignal) {
      const float luma = 0.2126f * mapped[0] +
          0.7152f * mapped[1] + 0.0722f * mapped[2];
      for(int channel = 0; channel < 3; channel++)
    ...
    if(adaptiveHero) {
      pixels[i * 3 + 0] = static_cast<unsigned char>(
          255.0f * clampFloat(mapped[2], 0.0f, 1.0f) + 0.5f);
      pixels[i * 3 + 1] = static_cast<unsigned char>(
          255.0f * clampFloat(mapped[1], 0.0f, 1.0f) + 0.5f);
      pixels[i * 3 + 2] = static_cast<unsigned char>(
          255.0f * clampFloat(mapped[0], 0.0f, 1.0f) + 0.5f);
    } else {
...
void sortPositiveValues(std::vector<float> *values)
{
...
float positiveQuantile(const std::vector<float> &values, float quantile)
{
  if(values.empty())
    return 0.0f;
  const double coordinate = double(stellarClamp(quantile, 0.0f, 1.0f)) *
      double(values.size() - 1);
  const size_t left = size_t(std::floor(coordinate));
  const size_t right = std::min(left + 1, values.size() - 1);
  const float fraction = float(coordinate - double(left));
...
struct ViewSpec {
  ...
  StellarPhysicalOpticalParametersV076 physical_optical;
  float display_brightness;
  std::string render_program_name;
  int render_program;
  StellarHeroDisplayContractV081 hero_display;
  std::string hero_display_contract_sha256;
};
...
std::vector<ViewSpec> loadViewManifest(const std::string &filename)
{
  ...
  std::string line;
  while(std::getline(input, line)) {
    if(line == "# schema=stellar_gpu_view_manifest_v053k") {
      schemaSeen = true;
      continue;
    }
    ...
    if(!schemaSeen)
      throw std::runtime_error(
          "View manifest data precedes stellar_gpu_view_manifest_v053k schema.");
    std::istringstream parser(line);
    ViewSpec view;
    ...
    if(!(parser >> view.index >> view.scene_path >> view.palette_path >> view.output_prefix >>
         ...
         view.display_brightness >> renderProgram >>
         view.hero_display.reference_quantile >>
         view.hero_display.response_floor_fraction >>
         view.hero_display.material_display_gain >>
         view.hero_display.outflow_display_gain >>
         view.hero_display_contract_sha256))
    ...
    view.render_program_name = renderProgram;
    view.render_program = stellarRenderProgramFromNameV081(renderProgram);
    ...
    view.physical_optical.profile =
        stellarPhysicalOpticalProfileFromNameV078(physicalOpticalProfile);
    std::string extra;
    if(parser >> extra)
    ...
    if(!stellarGpuProfileContractValidV053k(
           view.render_program, view.transfer_mode, view.palette_profile,
    ...
    const bool physical = view.physical_transfer.channel !=
        STELLAR_PHYSICAL_CHANNEL_OPTICAL_V071;
    if(view.render_program == STELLAR_RENDER_PROGRAM_INVALID_V081)
      throw std::runtime_error("Invalid stellar render program: " + line);
    if(view.render_program ==
           STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 &&
       (view.physical_transfer.channel !=
            STELLAR_PHYSICAL_CHANNEL_ROTATIONAL_FRACTION_V071 ||
        view.physical_transfer.scale != STELLAR_PHYSICAL_SCALE_LINEAR_V071 ||
        view.physical_transfer.range_min < 0.0f ||
        !(view.physical_transfer.range_max >
          view.physical_transfer.range_min) ||
        view.physical_transfer.range_max > 1.0f ||
        view.reconstruction_mode != STELLAR_RECONSTRUCTION_VORONOI ||
        !(view.physical_optical.color_gamma > 0.0f) ||
        (view.physical_optical.color_invert != 0 &&
         view.physical_optical.color_invert != 1) ||
        !stellarHeroDisplayContractValidV081(view.hero_display) ||
        !stellarHeroDisplayContractSha256ValidV081(
            view.hero_display_contract_sha256)))
      throw std::runtime_error("Invalid hero v081 projection contract: " + line);
    ...
    if(physical &&
       ...
       (!(view.physical_optical.density_support_log10_high >
        ...
        (view.physical_optical.color_invert != 0 &&
         view.physical_optical.color_invert != 1)))
      throw std::runtime_error(
          "Invalid density moment parameters: " + line);
  ...
  if(!schemaSeen)
    throw std::runtime_error(
        "View manifest lacks stellar_gpu_view_manifest_v053k schema.");
  if(views.empty())
                       ...
                       bool expectRaysOnly)
{
  ...
  if(candidate.num_cells != base.num_cells ||
     ...
     (candidate.flags & AREPO_STELLAR_ENTRY_RELATIVE_RAY_LIMIT_V080) !=
         (base.flags & AREPO_STELLAR_ENTRY_RELATIVE_RAY_LIMIT_V080) ||
  ...
  try {
    if(argc != 4) {
      std::cerr << "Usage: " << argv[0]
                << " view_manifest_v053k.tsv output_directory render_label\n";
      return 2;
    }
    ...
    if(!isA100 && !isA10 && !isA40 && !is2080Ti && !is3090 && !isL40S)
      throw std::runtime_error(
          "v053k supports only validated A100, A10, A40, RTX 2080 Ti, RTX 3090, and L40S devices.");
    ...
    const bool legacyAbsoluteRayLimit = baseHeader.ray_max_t > 0.0 &&
        (baseHeader.flags &
         AREPO_STELLAR_ENTRY_RELATIVE_RAY_LIMIT_V080) == 0;
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++)
      if(views[viewNumber].render_program ==
             STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 &&
         legacyAbsoluteRayLimit)
        throw std::runtime_error(
            "v081 rejects legacy absolute rayMaxT payloads; re-export with "
            "rayMaxT=0 or stellarGpuRayTraversalLengthCm.");
    ...
    scene.scene_flags = baseHeader.flags;
    ...
    for(size_t viewNumber = 0; viewNumber < views.size(); viewNumber++) {
      ...
      scene.ray_max_t = viewHeader.ray_max_t;
      scene.scene_flags = viewHeader.flags;
      ...
      scene.render_program = view.render_program;
      scene.hero_display = view.hero_display;
      ...
      std::vector<float> materialPeakDensities;
      std::vector<float> outflowPeakFluxes;
      if(view.render_program ==
         STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081) {
        materialPeakDensities.reserve(gpuResults.size());
        outflowPeakFluxes.reserve(gpuResults.size());
      }
      bool viewValid = true;
      for(size_t i = 0; i < gpuResults.size(); i++) {
        ...
        if(view.render_program ==
           STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081) {
          materialPeakDensities.push_back(
              gpuResults[i].hero_surface.material_peak_density_g_cm3);
          outflowPeakFluxes.push_back(
              gpuResults[i].hero_surface.outflow_peak_flux_g_cm2_s);
        }
        viewValid = viewValid &&
      ...
      sortPositiveValues(&materialPeakDensities);
      sortPositiveValues(&outflowPeakFluxes);
      const float materialPeakDensityP50 = positiveQuantile(
          materialPeakDensities, 0.50f);
      const float materialPeakDensityP90 = positiveQuantile(
          materialPeakDensities, 0.90f);
      const float materialPeakDensityP95 = positiveQuantile(
          materialPeakDensities, 0.95f);
      const float outflowPeakFluxP50 = positiveQuantile(
          outflowPeakFluxes, 0.50f);
      const float outflowPeakFluxP90 = positiveQuantile(
          outflowPeakFluxes, 0.90f);
      const float outflowPeakFluxP95 = positiveQuantile(
          outflowPeakFluxes, 0.95f);
      const float materialDisplayReference = positiveQuantile(
          materialPeakDensities, view.hero_display.reference_quantile);
      const float outflowDisplayReference = positiveQuantile(
          outflowPeakFluxes, view.hero_display.reference_quantile);
      ...
      writeTga(outputPrefix + ".tga", gpuResults,
               ...
               view.exposure, view.black_point, view.saturation,
               view.display_brightness, view.render_program,
               view.physical_optical, view.hero_display,
               materialDisplayReference, outflowDisplayReference);
      ...
      report << "renderer_version=stellar_v053k\n";
      report << "view_manifest_schema=stellar_gpu_view_manifest_v053k\n";
      report << "display_encoding="
             << (view.render_program ==
                     STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 ?
                 "camera_lab_direct_rgb" : "arepo_pow_1_over_2_3") << "\n";
      ...
      report << "physical_emission_signal_floor="
             << view.physical_optical.emission_signal_floor << "\n";
      report << "render_program=" << view.render_program_name << "\n";
      report << "projection_reference_quantile="
             << view.hero_display.reference_quantile << "\n";
      report << "projection_response_floor_fraction="
             << view.hero_display.response_floor_fraction << "\n";
      report << "projection_material_display_gain="
             << view.hero_display.material_display_gain
             << "\n";
      report << "projection_outflow_display_gain="
             << view.hero_display.outflow_display_gain << "\n";
      report << "projection_display_contract_sha256="
             << view.hero_display_contract_sha256 << "\n";
      report << "projection_material_peak_display_reference_g_cm3="
             << materialDisplayReference << "\n";
      report << "projection_outflow_peak_display_reference_g_cm2_s="
             << outflowDisplayReference << "\n";
      report << "projection_material_peak_density_p50_g_cm3="
             << materialPeakDensityP50 << "\n";
      report << "projection_material_peak_density_p90_g_cm3="
             << materialPeakDensityP90 << "\n";
      report << "projection_material_peak_density_p95_g_cm3="
             << materialPeakDensityP95 << "\n";
      report << "projection_outflow_peak_flux_p50_g_cm2_s="
             << outflowPeakFluxP50 << "\n";
      report << "projection_outflow_peak_flux_p90_g_cm2_s="
             << outflowPeakFluxP90 << "\n";
      report << "projection_outflow_peak_flux_p95_g_cm2_s="
             << outflowPeakFluxP95 << "\n";
      report << "physical_color_aggregation="
             << (view.render_program ==
                     STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 ?
                 "post_ray_view_normalized_dominant_cell_projection" :
                 (stellarPhysicalOpticalUsesCompositeMomentsV078(
                     view.physical_optical.profile) ?
                 "post_ray_normalized_scalar_amplitude_composite_coverage" :
                 (stellarPhysicalOpticalUsesNormalizedMomentsV077(
                     view.physical_optical.profile) ?
                 "post_ray_normalized_scalar_amplitude_coverage" :
                 (stellarPhysicalOpticalUsesMomentsV076(
                      view.physical_optical.profile) ?
                    "post_ray_scalar_moment" : "per_sample_rgb")))) << "\n";
      report << "physical_support="
             << (view.render_program ==
                     STELLAR_RENDER_PROGRAM_HERO_ADAPTIVE_COPPER_BLUE_V081 ?
                 "per_ray_peak_material_or_coherent_outflow_cell" :
                 (stellarPhysicalOpticalUsesCompositeMomentsV078(
                     view.physical_optical.profile) ?
                 "density_x_merger_disk_polar_union" : "retained")) << "\n";
      ...
      report << "ray_limit_mode="
             << ((viewHeader.flags &
                  AREPO_STELLAR_ENTRY_RELATIVE_RAY_LIMIT_V080) != 0 ?
                 "entry_relative_traversal" :
                 (viewHeader.ray_max_t > 0.0 ?
                  "legacy_absolute" : "unlimited")) << "\n";
      report << "ray_limit_value=" << viewHeader.ray_max_t << "\n";
      ...
      std::cout << "GPU_VIEW_OK index=" << view.index
                ...
                << " program=" << view.render_program_name
    ...
    summary << "renderer_version=stellar_v053k\n";
    summary << "view_manifest_schema=stellar_gpu_view_manifest_v053k\n";
    summary << "profile_contract=manifest_bound\n";
    summary << "physical_channel_contract=v072_all_24\n";
    summary << "physical_optical_contract=v078_legacy_v072_through_composite_moment_v078\n";
    summary << "render_program_contract=v081_retained_v078_or_adaptive_copper_blue\n";
    summary << "hero_components=material_peak_density,material_rotation,hot_support,outflow_peak_flux\n";
    summary << "hero_projection=per_ray_dominant_cell\n";
    summary << "hero_static_structure_weights=false\n";
    summary << "hero_false_color_integrated_as_radiance=false\n";
    summary << "hero_reference_scope=per_view_positive_ray_quantile\n";
    summary << "hero_display_encoding=camera_lab_direct_rgb\n";
    summary << "physical_palette=copper_blue\n";
    summary << "retained_display_encoding=arepo_pow_1_over_2_3\n";
