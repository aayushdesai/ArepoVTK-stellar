...
void setMotionBudgetOption(const std::string &name, const std::string &text,
                           StellarDirectorMotionBudgetV061 *budget)
{
  const double value = parseFinite(text, name);
...
int main(int argc, char **argv)
{
  try {
    ...
    StellarDirectorMotionBudgetV061 motion_budget = {0.25, 1.0, 5.0};
    ...
    if(!stellarDirectorValidMotionBudgetV061(motion_budget))
    ...
    std::string error;
    if(!stellarDirectorLoadLandmarksV061(landmarks_input, &landmarks, &error))
      throw std::runtime_error(error);
    if(!stellarDirectorLoadShotsV061(shots_input, &shots, &error))
      throw std::runtime_error(error);
    StellarDirectorResultV061 result;
    if(!stellarDirectorBuildV061(landmarks, shots, filter, motion_budget,
                                 &result, &error))
      throw std::runtime_error(error);
    const StellarDirectorMotionAssessmentV061 motion_assessment =
        stellarDirectorAssessMotionV061(result, motion_budget);
    ...
    std::string manifest = stellarDirectorManifestV061(
    ...
    if(path_given && motion_assessment.passed)
      writeNoClobber(path_path, stellarDirectorPathTableV061(
          result, landmarks_path, shots_path), "camera path");
    writeNoClobber(diagnostics_path,
                   stellarDirectorDiagnosticsTableV061(result),
                   "diagnostics");
    writeNoClobber(preview_path, stellarDirectorPreviewSvgV061(result),
                   "preview");
    writeNoClobber(manifest_path, manifest, "manifest");
    if(path_given && !motion_assessment.passed) {
      std::cerr << "STELLAR_CAMERA_DIRECTOR_V061_MOTION_REJECTED roll_deg_per_frame="
    ...
    std::cout << "STELLAR_CAMERA_DIRECTOR_V061_OK rows="
              << result.path.size() << " shots=" << shots.size()
              << " dry_run=" << (dry_run ? "true" : "false") << "\n";
    return 0;
  } catch(const std::exception &error) {
    std::cerr << "STELLAR_CAMERA_DIRECTOR_V061_ERROR " << error.what()
              << "\n";
    return 1;
  }
}
