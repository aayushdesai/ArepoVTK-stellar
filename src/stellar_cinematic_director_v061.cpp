...
bool poseForShot(const StellarCameraState &state,
                 const StellarDirectorShotV061 &shot,
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    StellarDirectorLandmarkRowV061 row = {};
    ...
    std::string *error)
{
  ...
  while(std::getline(input, line)) {
    ...
    if(start == std::string::npos)
      continue;
    if(line[start] == '#') {
      if(line.find("schema=stellar_cinematic_shots_v061", start) !=
         std::string::npos)
        schema_seen = true;
      continue;
    }
    ...
    if(!shots->empty()) {
      const StellarDirectorShotV061 &previous = shots->back();
  ...
  if(!schema_seen)
    return fail(error, "Missing schema=stellar_cinematic_shots_v061.");
  if(shots->empty())
    ...
    StellarDirectorResultV061 *result, std::string *error)
{
  if(!result || landmarks.empty() || shots.empty() ||
     ...
     !stellarDirectorValidMotionBudgetV061(budget))
  ...
  for(std::size_t index = 0; index < landmarks.size(); ++index) {
    ...
    StellarDirectorPathRowV061 row = {};
  ...
  for(std::size_t index = 0; index < result->path.size(); ++index) {
    const StellarDirectorPathRowV061 &row = result->path[index];
    StellarDirectorDiagnosticRowV061 diagnostic = {};
    ...
    if(index > 0) {
      const StellarDirectorPathRowV061 &previous = result->path[index - 1];
...
StellarDirectorMotionAssessmentV061 stellarDirectorAssessMotionV061(
    ...
    const StellarDirectorMotionBudgetV061 &budget)
{
  StellarDirectorMotionAssessmentV061 assessment = {};
  assessment.passed = stellarDirectorValidMotionBudgetV061(budget);
    ...
    const std::string &landmarks_path, const std::string &shots_path)
{
  ...
  output << "# schema=stellar_camera_path_v055\n"
         << "# director_schema=stellar_cinematic_director_v061\n"
...
{
  std::ostringstream output;
  output << "# schema=stellar_camera_diagnostics_v061\n"
    ...
    bool dry_run)
{
  std::ostringstream output;
  output << std::setprecision(17)
         << "schema=stellar_cinematic_direction_manifest_v061\n"
  ...
  for(std::size_t index = 0; index < shots.size(); ++index) {
    ...
    output << "shot." << index << ".name=" << shot.name << "\n"
           ...
           << stellarDirectorExtentSourceNameV061(shot.extent_source) << "\n"
           << "shot." << index << ".screen_half_extent_override_cm="
           << (shot.has_scale_override ?
               shot.screen_half_extent_override_cm : 0.0) << "\n"
           << "shot." << index << ".transition_kind="
           << stellarDirectorTransitionKindNameV061(shot.transition_kind)
           << "\n"
           ...
           << "shot." << index << ".easing="
           << (shot.transition_kind == STELLAR_DIRECTOR_TRANSITION_CONTINUOUS ?
               stellarDirectorEasingNameV061(shot.easing) : "-") << "\n";
  }
  return output.str();
}
