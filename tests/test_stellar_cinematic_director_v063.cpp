...
#include "stellar_cinematic_director_v063.h"
...
const char *calibrationText()
{
  return
      ...
      "105 north_lobe largest_component_area 0.078 0.25 55 dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd 3333333333333333333333333333333333333333333333333333333333333333\n"
      "106 north_lobe largest_component_area 0.023 0.25 70 eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee 3333333333333333333333333333333333333333333333333333333333333333\n";
}

bool loadLandmarks(const std::string &text,
                   std::vector<StellarDirectorLandmarkRowV063> *rows,
                   std::string *error)
{
  std::istringstream input(text);
  return stellarDirectorLoadLandmarksV063(input, rows, error);
}

bool loadShots(const std::string &text,
               std::vector<StellarDirectorShotV063> *shots,
               std::string *error)
{
  std::istringstream input(text);
  return stellarDirectorLoadShotsV063(input, shots, error);
}
...
{
  std::istringstream input(text);
  return stellarDirectorLoadVisualCalibrationV063(
      input, calibration, error);
}

bool finiteDiagnostics(const StellarDirectorResultV063 &result)
{
  for(std::size_t index = 0; index < result.diagnostics.size(); ++index) {
    const StellarDirectorDiagnosticRowV063 &row = result.diagnostics[index];
    if(!std::isfinite(row.center_speed_cm_per_s) ||
       ...
       !std::isfinite(row.visual_source_screen_half_extent_cm) ||
       !std::isfinite(row.visual_absolute_target_half_extent_cm) ||
...
int main()
{
  std::vector<StellarDirectorLandmarkRowV063> landmarks;
  std::vector<StellarDirectorShotV063> shots;
  std::vector<StellarDirectorVisualCalibrationRowV063> calibration;
  std::string error;
  ...
  const std::vector<StellarDirectorShotV063> valid_shots = shots;
  const std::vector<StellarDirectorVisualCalibrationRowV063>
  ...
  StellarDirectorMotionBudgetV063 loose_budget = {};
  ...
  StellarDirectorResultV063 first;
  StellarDirectorResultV063 second;
  assert(stellarDirectorBuildV063(landmarks, shots, calibration, filter,
                                  loose_budget,
                                  &first, &error));
  assert(stellarDirectorBuildV063(landmarks, shots, calibration, filter,
                                  loose_budget,
  ...
  const double opening_absolute_target = 75.0 * opening_factor;
  assert(std::abs(first.path[0].pose.screen_half_extent_cm -
                  opening_absolute_target) < 1.0e-12);
  assert(first.path[0].visual_calibration_applied == 1);
  assert(first.path[0].visual_metric ==
         STELLAR_DIRECTOR_VISUAL_PERCEPTUAL_AREA);
  assert(std::abs(first.path[0].visual_scale_factor -
                  opening_absolute_target / 60.0) <
         1.0e-12);
  assert(std::abs(first.path[0].visual_source_screen_half_extent_cm - 75.0) <
         1.0e-12);
  assert(std::abs(first.path[0].visual_absolute_target_half_extent_cm -
                  opening_absolute_target) < 1.0e-12);
  const double approach_end_factor = std::sqrt(0.36 / 0.45);
  const double approach_end_absolute_target = 90.0 * approach_end_factor;
  const double expected_time_interpolated_target = std::exp(
      0.75 * std::log(opening_absolute_target) +
      0.25 * std::log(approach_end_absolute_target));
  assert(std::abs(first.path[1].visual_absolute_target_half_extent_cm -
                  expected_time_interpolated_target) < 1.0e-12);
  assert(std::abs(first.path[1].visual_interpolation_fraction - 0.25) <
         1.0e-12);
  assert(std::abs(first.path[4].target_screen_half_extent_cm -
                  45.0 * std::sqrt(0.81 / 0.45)) <
         1.0e-12);
  assert(first.path[5].visual_absolute_target_half_extent_cm >
         first.path[6].visual_absolute_target_half_extent_cm);

  std::vector<StellarDirectorVisualCalibrationRowV063> iterative =
      calibration;
  iterative[0].source_screen_half_extent_cm =
      first.path[0].pose.screen_half_extent_cm;
  iterative[0].observed_fill = 0.8;
  iterative[0].target_fill = 0.4;
  iterative[0].source_camera_path_sha256 =
      "4444444444444444444444444444444444444444444444444444444444444444";
  StellarDirectorResultV063 iterative_result;
  assert(stellarDirectorBuildV063(landmarks, shots, iterative, filter,
                                  loose_budget, &iterative_result, &error));
  assert(std::abs(
      iterative_result.path[0].visual_absolute_target_half_extent_cm -
      first.path[0].pose.screen_half_extent_cm * std::sqrt(2.0)) < 1.0e-12);
  assert(std::abs(iterative_result.path[0].pose.screen_half_extent_cm -
                  first.path[0].pose.screen_half_extent_cm *
                      std::sqrt(2.0)) < 1.0e-12);
  ...
  const StellarDirectorMotionAssessmentV063 loose_assessment =
      stellarDirectorAssessMotionV063(first, loose_budget);
  assert(loose_assessment.passed);
  assert(loose_assessment.declared_cut_count == 1);
  StellarDirectorMotionBudgetV063 tight_budget = {};
  ...
  const StellarDirectorMotionAssessmentV063 tight_assessment =
      stellarDirectorAssessMotionV063(first, tight_budget);
  ...
  const std::string path1 = stellarDirectorPathTableV063(
      first, "landmarks.tsv", "shots.tsv", "visual.tsv");
  const std::string path2 = stellarDirectorPathTableV063(
      second, "landmarks.tsv", "shots.tsv", "visual.tsv");
  assert(path1 == path2);
  assert(stellarDirectorDiagnosticsTableV063(first) ==
         stellarDirectorDiagnosticsTableV063(second));
  const std::string manifest = stellarDirectorManifestV063(
  ...
  assert(manifest.find("visual_calibration.0.source_frame_sha256=aaaaaaaa") !=
         std::string::npos);
  assert(manifest.find(
             "visual_calibration.0.source_screen_half_extent_cm=75") !=
         std::string::npos);
  assert(manifest.find(
             "visual_calibration.0.source_camera_path_sha256=11111111") !=
         std::string::npos);
  assert(manifest ==
         stellarDirectorManifestV063(
             first, shots, calibration, filter, loose_budget,
             loose_assessment, "landmarks.tsv", "shots.tsv", "visual.tsv",
             false));
  assert(manifest == stellarDirectorManifestV063(
             second, shots, calibration, filter, loose_budget,
             stellarDirectorAssessMotionV063(second, loose_budget),
             "landmarks.tsv", "shots.tsv", "visual.tsv", false));
  const std::string iterative_manifest = stellarDirectorManifestV063(
      iterative_result, shots, iterative, filter, loose_budget,
      stellarDirectorAssessMotionV063(iterative_result, loose_budget),
      "landmarks.tsv", "shots.tsv", "visual_iterative.tsv", false);
  assert(iterative_manifest.find(
      "visual_calibration.0.source_camera_path_sha256=44444444") !=
      std::string::npos);
  const std::string diagnostics = stellarDirectorDiagnosticsTableV063(first);
  ...
  assert(diagnostics.find("visual_source_screen_half_extent_cm") !=
         std::string::npos);
  assert(diagnostics.find("visual_absolute_target_half_extent_cm") !=
         std::string::npos);
  ...
  assert(diagnostics.find("required_zoom_to_target_percent") !=
         std::string::npos);
  const std::string preview = stellarDirectorPreviewSvgV063(first);
  assert(preview == stellarDirectorPreviewSvgV063(second));
  ...
  std::vector<StellarDirectorShotV063> elevated = shots;
  elevated[0].elevation_radians += 0.2;
  StellarDirectorResultV063 elevated_result;
  assert(stellarDirectorBuildV063(landmarks, elevated, calibration, filter,
                                  loose_budget,
                                  &elevated_result, &error));
  assert(std::abs(elevated_result.path[0].pose.position[2] -
                  first.path[0].pose.position[2]) > 1.0e-6);

  std::vector<StellarDirectorShotV063> filtered_shots = shots;
  filtered_shots[1].extent_source = STELLAR_DIRECTOR_FILTERED_EXTENTS;
  StellarDirectorResultV063 filtered_result;
  assert(stellarDirectorBuildV063(landmarks, filtered_shots, calibration,
                                  filter, loose_budget, &filtered_result,
                                  &error));
  assert(std::abs(filtered_result.path[4].target_screen_half_extent_cm -
                  first.path[4].target_screen_half_extent_cm) < 1.0e-12);
  assert(std::abs(filtered_result.path[4].visual_scale_factor -
                  first.path[4].visual_scale_factor) > 1.0e-6);

  StellarDirectorMotionBudgetV063 limited_budget = {};
  ...
  StellarDirectorResultV063 limited_result;
  assert(stellarDirectorBuildV063(landmarks, shots, calibration, filter,
                                  limited_budget,
                                  &limited_result, &error));
  const StellarDirectorMotionAssessmentV063 limited_assessment =
      stellarDirectorAssessMotionV063(limited_result, limited_budget);
  ...
  StellarDirectorMotionBudgetV063 no_lag_budget = limited_budget;
  no_lag_budget.max_final_target_scale_error_percent = 0.0;
  const StellarDirectorMotionAssessmentV063 no_lag_assessment =
      stellarDirectorAssessMotionV063(limited_result, no_lag_budget);
  ...
  std::vector<StellarDirectorVisualCalibrationRowV063> invalid_calibration;
  const std::string missing_visual_schema =
      "100 approach perceptual_area 0.5 0.4 75 "
  ...
  const std::string bad_visual_hash =
      "# schema=stellar_visual_framing_calibration_v063\n"
      "100 approach perceptual_area 0.5 0.4 75 not-a-hash "
      "1111111111111111111111111111111111111111111111111111111111111111\n";
  ...
  const std::string bad_source_half_extent =
      "# schema=stellar_visual_framing_calibration_v063\n"
      "100 approach perceptual_area 0.5 0.4 0 "
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
      "1111111111111111111111111111111111111111111111111111111111111111\n";
  assert(!loadCalibration(bad_source_half_extent, &invalid_calibration,
                          &error));
  assert(error.find("source half extent") != std::string::npos);

  const std::string bad_source_path_hash =
      "# schema=stellar_visual_framing_calibration_v063\n"
      "100 approach perceptual_area 0.5 0.4 75 "
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
      "not-a-path-hash\n";
  assert(!loadCalibration(bad_source_path_hash, &invalid_calibration,
                          &error));
  assert(error.find("Invalid visual calibration") != std::string::npos);
  ...
  const std::string mixed_visual_metric =
      ...
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
      "1111111111111111111111111111111111111111111111111111111111111111\n"
      "102 approach perceptual_bbox 0.5 0.4 90 "
  ...
  std::vector<StellarDirectorVisualCalibrationRowV063> missing_shot =
      valid_calibration;
  missing_shot.erase(missing_shot.begin() + 4, missing_shot.end());
  StellarDirectorResultV063 invalid_result;
  assert(!stellarDirectorBuildV063(landmarks, valid_shots, missing_shot,
                                   filter, loose_budget, &invalid_result,
                                   &error));
  assert(error.find("Every shot requires") != std::string::npos);

  std::vector<StellarDirectorVisualCalibrationRowV063> wrong_snapshot =
      valid_calibration;
  wrong_snapshot[0].snapshot = 99;
  assert(!stellarDirectorBuildV063(landmarks, valid_shots, wrong_snapshot,
                                   filter, loose_budget, &invalid_result,
                                   &error));
  assert(error.find("bind a shot landmark") != std::string::npos);

  std::vector<StellarDirectorVisualCalibrationRowV063> unknown_shot =
      valid_calibration;
  StellarDirectorVisualCalibrationRowV063 extra = valid_calibration.back();
  extra.shot_name = "unknown";
  unknown_shot.push_back(extra);
  assert(!stellarDirectorBuildV063(landmarks, valid_shots, unknown_shot,
                                   filter, loose_budget, &invalid_result,
                                   &error));
  assert(error.find("unknown shot") != std::string::npos);

  std::vector<StellarDirectorLandmarkRowV063> invalid_landmarks;
  ...
  std::cout << "STELLAR_CINEMATIC_DIRECTOR_V063_OK\n";
  return 0;
}
