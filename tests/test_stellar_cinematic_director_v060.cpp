...
bool loadLandmarks(const std::string &text,
                   std::vector<StellarDirectorLandmarkRowV060> *rows,
                   std::string *error)
{
  std::istringstream input(text);
  return stellarDirectorLoadLandmarksV060(input, rows, error);
}

bool loadShots(const std::string &text,
               std::vector<StellarDirectorShotV060> *shots,
               std::string *error)
{
  std::istringstream input(text);
  return stellarDirectorLoadShotsV060(input, shots, error);
}

bool finiteDiagnostics(const StellarDirectorResultV060 &result)
{
...
int main()
{
  std::vector<StellarDirectorLandmarkRowV060> landmarks;
  std::vector<StellarDirectorShotV060> shots;
  std::string error;
  ...
  StellarDirectorMotionBudgetV060 loose_budget = {};
  ...
  StellarDirectorResultV060 first;
  StellarDirectorResultV060 second;
  assert(stellarDirectorBuildV060(landmarks, shots, filter, loose_budget,
                                  &first, &error));
  assert(stellarDirectorBuildV060(landmarks, shots, filter, loose_budget,
                                  &second, &error));
  ...
  const StellarDirectorMotionAssessmentV060 loose_assessment =
      stellarDirectorAssessMotionV060(first, loose_budget);
  assert(loose_assessment.passed);
  StellarDirectorMotionBudgetV060 tight_budget = {};
  ...
  const StellarDirectorMotionAssessmentV060 tight_assessment =
      stellarDirectorAssessMotionV060(first, tight_budget);
  ...
  const std::string path1 = stellarDirectorPathTableV060(
      first, "landmarks.tsv", "shots.tsv");
  const std::string path2 = stellarDirectorPathTableV060(
      second, "landmarks.tsv", "shots.tsv");
  assert(path1 == path2);
  assert(stellarDirectorDiagnosticsTableV060(first) ==
         stellarDirectorDiagnosticsTableV060(second));
  const std::string manifest = stellarDirectorManifestV060(
  ...
  assert(manifest.find("shot.1.transition_end_snapshot=104") !=
         std::string::npos);
  assert(manifest ==
         stellarDirectorManifestV060(
             first, shots, filter, loose_budget, loose_assessment,
             "landmarks.tsv", "shots.tsv", false));
  assert(manifest == stellarDirectorManifestV060(
             second, shots, filter, loose_budget,
             stellarDirectorAssessMotionV060(second, loose_budget),
             "landmarks.tsv", "shots.tsv", false));
  const std::string diagnostics = stellarDirectorDiagnosticsTableV060(first);
  ...
  const std::string preview = stellarDirectorPreviewSvgV060(first);
  assert(preview == stellarDirectorPreviewSvgV060(second));
  ...
  std::vector<StellarDirectorShotV060> elevated = shots;
  elevated[0].elevation_radians += 0.2;
  StellarDirectorResultV060 elevated_result;
  assert(stellarDirectorBuildV060(landmarks, elevated, filter, loose_budget,
                                  &elevated_result, &error));
  assert(std::abs(elevated_result.path[0].pose.position[2] -
                  first.path[0].pose.position[2]) > 1.0e-6);

  std::vector<StellarDirectorShotV060> filtered_shots = shots;
  filtered_shots[1].extent_source = STELLAR_DIRECTOR_FILTERED_EXTENTS;
  StellarDirectorResultV060 filtered_result;
  assert(stellarDirectorBuildV060(landmarks, filtered_shots, filter,
  ...
  StellarDirectorMotionBudgetV060 limited_budget = {};
  ...
  StellarDirectorResultV060 limited_result;
  assert(stellarDirectorBuildV060(landmarks, shots, filter, limited_budget,
                                  &limited_result, &error));
  const StellarDirectorMotionAssessmentV060 limited_assessment =
      stellarDirectorAssessMotionV060(limited_result, limited_budget);
  ...
  StellarDirectorMotionBudgetV060 no_lag_budget = limited_budget;
  no_lag_budget.max_final_target_scale_error_percent = 0.0;
  const StellarDirectorMotionAssessmentV060 no_lag_assessment =
      stellarDirectorAssessMotionV060(limited_result, no_lag_budget);
  ...
  const std::string transition_outside_shot =
      "# schema=stellar_cinematic_shots_v060\n"
      "one 100 102 disk_edge merger 0 0 1.1 - - 1 raw - linear\n"
      "two 103 106 disk_edge merger 0 0 1.1 - - 1 raw 107 linear\n";
